...
QOptimizer optimizer=QOptimizer(1e-6, 100);
...
bool QSketch::load(QString fileName)
{
	...
	while(!textStream.atEnd())
	{
		...
		if(line[0]=="s")
		{
			width=line[1].toInt(); height=line[2].toInt();
			e.viewer.resize(QSize(width, height));
			isValidSketch=true;
		}
		if(!isValidSketch)continue;
		if(line[0]=="v")e.viewer+=line;
		else if(line[0]=="f")e.viewer+=line;
		else if(line[0]=="u")e.viewer+=line;
		else if(line[0]=="r")e.viewer+=line;
		else if(line[0]=="m")
		{
			int x=line[1].toInt();
			int y=line[2].toInt();
