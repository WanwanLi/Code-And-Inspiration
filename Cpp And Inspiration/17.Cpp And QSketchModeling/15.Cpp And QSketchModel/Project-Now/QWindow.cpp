...
QMenuBar* QWindow::newQMenuBar()
{
	...
	action=menu->addAction("Perturb Planes", canvas, &QCanvas::perturbPlanes, QKeySequence(tr("Ctrl+p")));
	action=menu->addAction("Minimize Energy", canvas, &QCanvas::minimizeEnergy, QKeySequence(tr("Ctrl+m")));
	action=menu->addAction("Create Relations", canvas, &QCanvas::createRelations, QKeySequence(tr("Ctrl+w")));
	action=menu->addAction("Set Camera Upward", canvas, &QCanvas::setCameraUpward, QKeySequence(tr("Ctrl+u")));
	action->setCheckable(true);
	action->setChecked(true);
	canvas->setCameraUpward();
