...
void QOpenCVWindow::timeout()
{
	...
	cv::cvtColor(originalPixels, processedPixels, CV_BGR2RGB);
	this->setPixmap(imageLabel[2], processedPixels, QImage::Format_RGB888);
	cv::cvtColor(processedPixels, processedPixels, CV_BGR2GRAY);
	cv::Canny(processedPixels, processedPixels, 50, 100, 3);
	...
	this->setPixmap(imageLabel[0], processedPixels, QImage::Format_Indexed8);
}