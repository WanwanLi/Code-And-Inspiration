...
{
	if(index<=2)return 2;
	if(path[index-4]==MOVE)return index-4;
	if(path[index-4]==LINE)return index-4;
	if(path[index-10]==CUBIC)return index-10;
}
int QStrokes::next(int index)
{
	if(index==0)return 2;
	if(path[index]==MOVE)return index+4;
	if(path[index]==LINE)return index+4;
	if(path[index]==CUBIC)return index+10;
}
void QStrokes::clear()
{
	this->index=0;
...
void QStrokes::setPath(int type)
{
	...
	else if(type==CUBIC)
	{
		...
		this->path<<p1.x()<<p1.y()<<p1.z();
		this->path<<p2.x()<<p2.y()<<p2.z();
		this->path<<p3.x()<<p3.y()<<p3.z();
	}
}
void QStrokes::removeLast()
{
	if(index<=2)return;
	if(path[index]==MOVE)this->removeLast(4);
	else if(path[index]==LINE)this->removeLast(4);
	else if(path[index]==CUBIC)this->removeLast(10);
}
