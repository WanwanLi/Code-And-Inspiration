...
QVector3D QGraph::getPoint(int index)
{
	...
	if((int)path[index]==CUBIC)return QVector3D(path[index+7], path[index+8], path[index+9]);
...
void QGraph::setPoint(int index, QVector3D& point)
{
	...
	else if((int)path[index]==CUBIC){path[index+7]=point.x(); path[index+8]=point.y(); path[index+9]=point.z();}
}
void QGraph::clear()
{
	this->vertices.clear();
