...
QSketch::QSketch(QCanvas* canvas)
{
	this->optimizer=new QOptimizer(1e-5, 100, 10);
	this->canvas=canvas;
}
...
{
	//this->optimizer->minimizePlaneEnergy();
	this->optimizer->sketch=this;
	this->optimizer->minimizeTotalEnergy();


	//this->minimizer->optimizer=this->optimizer;
...
{
	this->optimizer->drawMatchPoints(painter, index);
}
void QSketch::drawMarkers(QPainter& painter)
{
	this->optimizer->drawMarkers(painter);
	this->optimizer->drawRelations(painter);
}
void QSketch::createRelations()
{
	this->optimizer->createRelations();
}
void QSketch::perturbPlanes(qreal noise)
{
	this->optimizer->perturbPlanes(noise);
	this->clear();
	this->create();
	this->update();
}
...
{
	this->optimizer->energy.viewDistance=viewDistance;
}
void QSketch::clear()
{
	this->updatePainterPaths();
