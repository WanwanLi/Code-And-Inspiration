...
QOpenGLPolygon::QOpenGLPolygon(const QVector<QVector3D*>& quads, const QVector<QVector<qreal>>& coords)
{
	for(int i=0; i<quads.size(); this->loadPolygon(quads[i], coords[i++]));
...
{
	QVector3D pU0=(1-u)*p00+u*p10;
	QVector3D pU1=(1-u)*p01+u*p11;
	return (1-v)*pU0+v*pU1;
}
