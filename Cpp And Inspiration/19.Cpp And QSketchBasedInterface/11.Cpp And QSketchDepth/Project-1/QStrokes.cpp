...
void QStrokes::addDepthPath(int index)
{
	...
	if(intersectDepthPath(positions, isDeleted, index, next(index)))
	{
		this->appendDepthPath(index, MOVE, positions[0]);
		for(int i=0; i<positions.size()-1; i++)
		{
			if(isDeleted[i])this->appendDepthPath(index, MOVE, positions[i+1]);
			else this->appendDepthPath(index, LINE, positions[i+1]);
		}
	}
}
bool QStrokes::intersectDepthPath(QVector<qreal>& positions, QVector<bool>& isDeleted, int index0, int index1)
{
    ...
    qreal depth0=(p0-viewer.eye()).length();
    qreal depth1=(p1-viewer.eye()).length();
	if(path[index1]==MOVE)return false;
	else if(path[index1]==LINE)
	{
		for(int i=2, j=next(i); j<path.size(); i=j, j=next(i))
		{
            int plane1=planeIndex(j); if(plane0==plane1||path[j]==MOVE)continue;
            qreal position, depth; minimizeDepth(p0, depth0, plane1); minimizeDepth(p1, depth1, plane1);
	...
	QVector<qreal> deletedPositions; 
	deletedPositions.clear();
	for(int i=0; i<planes.size(); i++)
	{
		positions.clear(); depths.clear();
		depths.insert(insert(positions, 0), depth0);
		depths.insert(insert(positions, 1), depth1);
...
qDebug()<<"depths: "<<depths;
        ...
        for(int i=0, index=index0; i<positions.size()-1; i++)
		{
...
qDebug()<<"depth("<<t0<<")="<<depth(index, t0);
            ...
            if(isEndPosition(positions, i)||equals(depth(index, t0), depths[i+0])&&equals(depth(index, t1), depths[i+1]));
...
qDebug()<<"MOVE "<<t0<<", "<<t1;
				...
				deletedPositions<<t0<<t1;
			}
	}
if(deletedPositions.size()==4)
{
qDebug()<<"deletedPositions : "<<deletedPositions;
}
	if(deletedPositions.size()==0)return false;
	positions.clear(); positions<<0<<1;  
	isDeleted.clear(); isDeleted<<false<<false;
	for(int i=0; i<deletedPositions.size(); i+=2)
	{
		qreal d0=QVecMath::clamp(deletedPositions[i+0], 0, 1);
		qreal d1=QVecMath::clamp(deletedPositions[i+1], 0, 1);
        qreal start=QVecMath::min(d0, d1);
        qreal end=QVecMath::max(d0, d1);
		if(equals(start, 1))continue;
		addDeletedPositions(positions, isDeleted, start, end);
	}
	return true;
}
void QStrokes::addDeletedPositions(QVector<qreal>& positions, QVector<bool>& isDeleted, qreal start, qreal end)
{
	if(positions.size()==2)
	{
		if(equals(start, 0))
		{
			isDeleted[0]=true; if(!equals(end, 1))isDeleted.insert(insert(positions, end), false);
		}
		else
		{
			isDeleted.insert(insert(positions, start), true);
			if(!equals(end, 1))isDeleted.insert(insert(positions, end), false);
		}
	}
}
...
void QStrokes::minimizeDepth(QVector3D position, qreal& depth, int plane)
{
	QVector3D eye=viewer.eye(), point=QVecMath::intersectPlane(eye, position-eye).normalized(), planes[plane]);
	qreal newDepth=(point-eye).length(); if(newDepth<depth)depth=newDepth;
}
