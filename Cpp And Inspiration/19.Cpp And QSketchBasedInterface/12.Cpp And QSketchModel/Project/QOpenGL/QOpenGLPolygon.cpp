...
void QOpenGLPolygon::addPolygonSide(QVector3D* quad, QVector<qreal> coords, QVector3D normal)
{
	for(int i=0, size=coords.size()/2; i<size; i++)
	{
		...
		this->positions<<p00<<p10<<p11<<p01; this->normals<<normal;
	}
}
...
QVector3D QOpenGLPolygon::getNormal(QVector3D* quad)
{
	QVector3D p00=quad[0], p01=quad[1], p10=quad[3];
	return cross(p01-p00, p10-p00).normalized();
}
...
void QOpenGLPolygon::loadPolygon(QVector3D* quad, const QVector<qreal>& coords)
{
	...
	QVector3D normal=getNormal(quad);
...
{
	QVector3D du=p10-p00, dv=p01-p00;
