...
void QOpenGLWindow::paintGL()
{
	...
	glCube->indexBuffer->bind();  glTexture->bind();
	glBindAttributeBuffer("texcoord", glCube->texcoordBuffer, 2);
	glBindAttributeBuffer("position", glCube->positionBuffer, 3);
	glBindAttributeBuffer("normal", glCube->normalBuffer, 3);
	glDrawElements(GL_TRIANGLES, glCube->indexLength, GL_UNSIGNED_SHORT, 0);
}
