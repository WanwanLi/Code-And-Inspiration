...
void QOpenGLWindow::initializeGL()
{
	...
	glObject=new QOpenGLObject("Teapot.obj");
...
void QOpenGLWindow::paintGL()
{
	...
	glModelMatrix.scale(0.125);
	glModelMatrix.translate(0.0, 0.0, 0.25);
	...
	glObject->indexBuffer->bind();  glTexture->bind();
	glBindAttributeBuffer("texcoord", glObject->texcoordBuffer, 2);
	glBindAttributeBuffer("position", glObject->positionBuffer, 3);
	glBindAttributeBuffer("normal", glObject->normalBuffer, 3);
	glDrawElements(GL_TRIANGLES, glObject->indexLength, GL_UNSIGNED_SHORT, 0);
}
