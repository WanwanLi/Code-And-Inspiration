...
var QEnergy::totalEnergy(const VectorXv& plane)
{
	Map<const MatrixXi> joints(jointVector.data(), jointDim, jointVector.size()/jointDim);
	Map<const MatrixXi> sketches(sketchVector.data(), 3, sketchVector.size()/3);
