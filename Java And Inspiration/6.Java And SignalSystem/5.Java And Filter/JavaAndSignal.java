...
class Frame_Signal extends Frame
{
	...
	{
		int length=1000;
		double w0=-10*Math.PI,w1=10*Math.PI,wc=Math.PI,k=-0.5,t0=0,t1=10;
		double[] w=Signal.X(w0,w1,length),x=w,y;
		double[] t=Signal.X(t0,t1,length);
		double[][] lowPassFilter=Signal.lowPassFilter(wc,k,w0,w1,length);
		double[][] freq=Signal.frequency(lowPassFilter);
		y=freq[0];
		y=freq[1];

		y=Signal.response(lowPassFilter,w0,w1,Signal.impulse(0,1,length),t0,t1);
/*
		y=Signal.exp(0.5,t);
		y=Signal.response(lowPassFilter,w0,w1,y,t0,t1);
		x=t;

		double[][] highPassFilter=Signal.highPassFilter(wc,k,w0,w1,length);
		freq=Signal.frequency(highPassFilter);
		y=freq[0];
		y=freq[1];

		y=Signal.response(highPassFilter,w0,w1,Signal.impulse(0,1,length),t0,t1);
		y=Signal.exp(0.5,t);
		y=Signal.response(highPassFilter,w0,w1,y,t0,t1);
		x=t;

		double[][] bandPassFilter=Signal.bandPassFilter(wc,5*wc,k,w0,w1,length);
		freq=Signal.frequency(bandPassFilter);
		y=freq[0];
		y=freq[1];

		y=Signal.response(bandPassFilter,w0,w1,Signal.impulse(0,1,length),t0,t1);
		x=t;
*/
...
class Signal
{
	...
	public static double[] response(double[][] filter,double w0,double w1,double[] impluse,double t0,double t1)
	{
		int length=impluse.length;
		double[] x=impluse;
		double[][] X=Laplace(0,w0,w1,length,x,t0,t1);
		double[][] Y=new double[length][];
		for(int i=0;i<length;i++)Y[i]=MUL(filter[i],X[i]);
		double[] y=LaplaceInverse(0,t0,t1,length,Y,w0,w1);
		return y;
	}
	public static double[][] lowPassFilter(double wl,double k,double w0,double w1,int length)
	{
		double[][] H=new double[length][];
		double dw=(w1-w0)/(length-1);
		for(int i=0;i<length;i++)
		{
			double wi=w0+i*dw;
			if(Math.abs(wi)<=wl)H[i]=e(k*wi);
			else H[i]=new double[]{0,0};
		}
		return H;
	}
	public static double[][] highPassFilter(double wh,double k,double w0,double w1,int length)
	{
		double[][] H=new double[length][];
		double dw=(w1-w0)/(length-1);
		for(int i=0;i<length;i++)
		{
			double wi=w0+i*dw;
			if(Math.abs(wi)>=wh)H[i]=e(k*wi);
			else H[i]=new double[]{0,0};
		}
		return H;
	}
	public static double[][] bandPassFilter(double wl,double wh,double k,double w0,double w1,int length)
	{
		double[][] H=new double[length][];
		double dw=(w1-w0)/(length-1);
		for(int i=0;i<length;i++)
		{
			double wi=w0+i*dw;
			if(Math.abs(wi)>=wl&&Math.abs(wi)<=wh)H[i]=e(k*wi);
			else H[i]=new double[]{0,0};
		}
		return H;
	}
