...
public class JavaAndFastFourierTransform2D
{
	public static void main(String[] args)
	{
		Frame_FastFourierTransform2D Frame_FastFourierTransform=new Frame_FastFourierTransform2D();
		Frame_FastFourierTransform.setVisible(true);
	}
}
class Frame_FastFourierTransform2D extends Frame
{
	public Image Image1,Image2;
	int imageWidth,imageHeight;
	public Frame_FastFourierTransform2D()
	{
		...
		try
		{
			Image1=Toolkit.getDefaultToolkit().getImage("JavaAndFastFourierTransform2D.jpg");
			...
			ImageOp1.getFastFourierTransform2D();
...
class ImageOp
{
	...
	public void getFastFourierTransform2D()
	{
		this.grey(1,1,1);
		FastFourierTransform2D FourierTransform=new FastFourierTransform2D(Pixels,ImageWidth,ImageHeight);
		FourierTransform.inverse();
		this.Pixels=FourierTransform.getPixels();
	}
...
class FastFourierTransform2D
{
	...
	public FastFourierTransform2D(int[] pixels,int imageWidth,int imageHeight)
	{
		...
		for(int i=0;i<m;i++)for(int j=0;j<n;j++)a[i][j]=new Complex((pixels[i*n+j]&255)+0.0,0.0);
	...
	public void translateToCenter()
	{
		...
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
			{
				Complex Em=Complex.e2PI(m,i*m/2);
				Complex En=Complex.e2PI(n,j*n/2);
				Complex Emn=Complex.MUL(Em,En);
				Complex E=new Complex(e(i,j),0.0);
				this.A[i][j]=Complex.MUL(A[i][j],E);
			}
		}
	}
	...
	public int[] getPixels()
	{
		...
		for(int i=0;i<imageHeight;i++)
		{
			for(int j=0;j<imageWidth;j++)
			{
				...
				int grey=(int)length;
				pixels[i*imageWidth+j]=255<<24|(grey<<16)|(grey<<8)|grey;
			}
		}
		return pixels;
	}
