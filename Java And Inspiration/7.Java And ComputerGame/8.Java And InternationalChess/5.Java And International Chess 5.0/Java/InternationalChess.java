...
import java.net.*;
import java.util.*;
public class InternationalChess extends Applet implements ActionListener,MouseListener,KeyListener
{
	class Position
	{
		public int i0;
	...
	class ChessBoard
	{
		...
		{
			for(int i=0;i<8;i++)for(int j=0;j<8;j++)Chess[i][j]=chess[i][j];
			Next=null;
		}
	}
	...
	public void paint(Graphics g)
	{
		...
		if(canMoveYellow){getAndSetPositionForYellow();	ChessBoardRecord1.add(chess);}
	}
	...
	private void getAndSetPositionForYellowKing()
	{
		...
		if(bestPosition==null){JOptionPane.showMessageDialog(null,"Yellow King Lose!");System.exit(0);}
	...
	private int getMaxScoreForYellow(int depth)
	{
		...
		PositionList PositionList1=new PositionList();
		this.getAllPosiblePositionsForYellow(PositionList1);
		if(depth>5||recursionCounter>10000)return PositionList1.BestPosition.Score;
		for(Position p=PositionList1.BestPosition;p!=null;p=p.Next)
		{
			int i0=p.i0;
			int j0=p.j0;
