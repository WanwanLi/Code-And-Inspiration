...
class TianShen
{
	...
	public void askPlayersChairNumber(int player, boolean isMultiplePlayers)
	{
		String players=getPlayersName(player);
		if(isMultiplePlayers)
		{
			if(isChinese)print(players+resources.getString(R.string.chair_nums));
			else print("What are the chair No.s of "+players);
		}
		else
		{
	...
	public void askPlayersToFindRepresentitive(int player)
	{
		String players=getPlayersName(player);
		if(isChinese)
		{
			println(players + resources.getString(R.string.represent));
			print
			(
		...
		{
			println(players + ", please chose someone to be your representitive.");
	...
	public void showAllPlayers(int[] players, int king)
	{
		...
		for(int i=1; i<players.length; i++)
		{
			...
			if(isChinese)text+="["+i+resources.getString(R.string.player)+player+isKing+isDied;
			else text+=("The charater of player ["+i+"] is: "+player+isKing+isDied);
		}
		this.setText(text);
	}
	...
	public void printNotMatchErrorMessage(int player)
	{
		String players=getPlayersName(player);
