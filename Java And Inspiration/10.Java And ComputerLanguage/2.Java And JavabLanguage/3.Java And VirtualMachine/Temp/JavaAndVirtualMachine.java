...
//this.showDoubleTable();
	...
	private void execute()
	{
		...
		else if(OP.equals("LINE"))
		{
			...
			else if(c=='@')x0=integerTable[integerTable[x0]];
			c=this.nextChar();
			int y0=this.nextInt();
			if(c=='&')y0=integerTable[y0];
			else if(c=='@')y0=integerTable[integerTable[y0]];
			c=this.nextChar();
			int x1=this.nextInt();
			if(c=='&')x1=integerTable[x1];
			else if(c=='@')x1=integerTable[integerTable[x1]];
			c=this.nextChar();
			int y1=this.nextInt();
			if(c=='&')y1=integerTable[y1];
			else if(c=='@')y1=integerTable[integerTable[y1]];
	...
	private Word scanNextWord()
	{
		...
		else
		{
			switch(c)
			{
				...
				else if(c=='=')return new Word(Word.ARI,Word.PRO);
				else{this.index--;return new Word(Word.ARI,Word.MUL);}
				case '/':
				if(index>=length)break;
				c=code.charAt(index++);
				if(c=='*'){this.isCode=false;break;}
				else if(c=='=')return new Word(Word.ARI,Word.QUO);
