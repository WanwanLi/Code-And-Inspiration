...
public class JavaAndVirtualMachine3D
{
	...
	{
		System.out.print("Java Flat Virtual Machine 3D>");
		Scanner scanner=new Scanner(System.in);
		String cmd=scanner.next();
		if(!cmd.equals("javab"))System.exit(0);
		String className=scanner.next();
		String parameters=scanner.next();
		generateVirtualMachineCode(className);
		VirtualMachine3D virtualMachine3D=new VirtualMachine3D();
		virtualMachine3D.config(className,parameters);
		new MainFrame(virtualMachine3D,400,400);
	}
	private static void generateVirtualMachineCode(String className)
	{
...
class VirtualMachine3D extends Applet
{
	...
	public void config(String className,String parameters)
	{
		...
		this.getVirtualMachineInstructions(className);
	...
	public void init()
	{
		...
		canvas3D.setBounds(-5,-5,screenWidth+5,screenHeight+5);
		this.add(canvas3D);
	...
	void add()
	{
		...
		Font3D font3D=new Font3D(new Font(null,50,Font.BOLD),new FontExtrusion());
		...
		this.transformGroup.addChild(TransformGroup2);
	...
	private void getVirtualMachineInstructions(String className)
	{
		try
		{
	...
	private void execute()
	{
		...
		else if(OP.equals("DEQI"))
		{
			char c=this.nextChar();
			int i=this.nextInt();
			this.integerTable[i]=Integer.parseInt(integerQueue.deQueue());
		}
		...
		else if(OP.equals("READI"))
		{
			...
			int i=this.nextInt();
			Scanner scanner=new Scanner(System.in);
			this.integerTable[i]=scanner.nextInt();
		}
		...
		else if(OP.equals("LINE"))
		{
			char c=this.nextChar();
			int x0=this.nextInt();
			if(c=='&')x0=integerTable[x0];
			c=this.nextChar();
			int y0=this.nextInt();
			if(c=='&')y0=integerTable[y0];
			c=this.nextChar();
			int x1=this.nextInt();
			if(c=='&')x1=integerTable[x1];
			c=this.nextChar();
			int y1=this.nextInt();
			if(c=='&')y1=integerTable[y1];
...
class SyntaxTranslator
{
	...
	private int identifierNumber,stringNumber;
	...
	private void getDeclarationOrAssignmentExpressionInstructions()
	{
		...
		else if(w0==Word.ID&&identifierTable[w1].Type==WordList.Key_String)
		{
			...
			this.index--;
			if(k0==Word.ID&&identifierTable[k1].Type==WordList.Key_public)this.getMethodAssignmentInstructions();
			else this.getStringAssignmentInstructions();
		}
	}
	...
	private void getThisExpressionInstructions()
	{
		...
		if(w0==Word.MRK&&w1==Word.DOT)
		{
			...
			if(w0==Word.ID)
			{
				...
				else if(thisMethod.equals("length"))
				{
					this.index++;
					String id0="&-1",id1="&-1";
					this.nextWord();
					if(identifierTable[w1].Type==WordList.Key_int)id0="&"+w1;
					else id1="&"+w1;
					this.index++;
					this.nextWord();
					if(identifierTable[w1].Type==WordList.Key_array)id1="&"+w1;
					else id0="&"+w1;
					this.instructionsQueue.enQueue("MOVLEN "+id0+","+id1);
					this.index++;
				}
				...
				else if(thisMethod.equals("fillPolygon"))
				{
					...
					else if(w0==Word.KEY&&w1==WordList.Key_new)
					{
						this.nextWord();
						if(w0==Word.KEY&&w1==WordList.Key_int)
						{
							int id=(identifierNumber++);
							this.instructionsQueue.enQueue("ARY &"+id);
							this.instructionsQueue.enQueue("MOV &"+id+", "+(id+1));
							this.getArrayAssignmentInstructions(id);
							this.instructionsQueue.enQueue("FPOLY &"+id);
							this.index--;
						}
					}
					this.index+=2;
	...
	private void getIntDeclarationInstructions()
	{
		...
		else if(w0==Word.ARI&&w1==Word.MOV)
		{
			...
			if(w0==Word.ID&&identifierTable[w1].Type==WordList.Key_public)this.getMethodAssignmentInstructions();
			else this.getArithmeticAssignmentInstructions();
			this.index++;
			this.getIntDeclarationInstructions();
		}
