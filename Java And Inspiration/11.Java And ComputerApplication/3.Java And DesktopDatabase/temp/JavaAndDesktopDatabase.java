...
import javax.swing.*;
class JavaAndDesktopDatabase
{
	public static void main(String[] args)
	{
		new DesktopDatabaseFrame();
	}
}
class DesktopDatabaseFrame extends JFrame
{
	public DesktopDatabaseFrame()
	{
		...
		DesktopDatabasePanel desktopDatabasePanel=new DesktopDatabasePanel(this,screenWidth,screenHeight);
...
class DesktopDatabasePanel extends JPanel implements KeyListener,MouseListener,MouseMotionListener,TextListener
{
	...
	public DesktopDatabasePanel(Frame screenFrame,int screenWidth,int screenHeight)
	{
	...
	public void mouseMoved(MouseEvent e)
	{
		...
		if(state==REGISTER)
		{
			...
			if(mouseIsInBounds(600,540,177,38))this.isOnRegisterButton=true;
	...
	private void drawRegisterFrame(Graphics g)
	{
		...
		String title="Java And DesktopDatabase";
		this.drawTitle(g,title,560,420);
		int x=550,y=465,w=276,h=25;
		this.drawPasswordTextField(g,x,y,w,h);
		if(isOnRegisterButton)g.drawImage(registerbutton_shineImage,600,540,this);
		else g.drawImage(registerbuttonImage,600,540,this);
		this.drawTitle(g,"Enter",650,560);
	}
		...
		while(k<l1)
		{
			for(int j=0;j<3;j++)
			{
				...
				if(sl>length)
				{
					...
					String s2=s.substring(length,sl);
					g.drawString(s1,sx,sy);
					g.drawString(s2,sx-10,sy+20);
				}
				else g.drawString(s,sx,sy);
				if(k>=l1)return;
			}
			y+=h;
	...
	private void drawAlterFrame(Graphics g)
	{
		...
		for(int i=beginCatalogPosition;i<l;i++)
		{
			...
			g.drawString(this.getStringLines(catalogFields[i*2+1],12)[0],x,y0+110);
			x+=dx;
			if(x>alterframeX+832)break;
		}
	}
