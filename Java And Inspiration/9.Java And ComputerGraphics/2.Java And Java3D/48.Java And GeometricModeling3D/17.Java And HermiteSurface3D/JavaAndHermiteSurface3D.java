...
class HermiteSurface3D extends Shape3D
{
	...
	private Point3d[] getCoordinates1(Point3d[][] ctrlPoints,Point3d[][] uCtrlVectors,Point3d[][] vCtrlVectors,double[] uKnots,double[] vKnots,int row,int column)
	{
	...
	{
		Point3d[] coordinates=new Point3d[row*column];
		double du=1.0/(row-1),dv=1.0/(column-1);
		for(int r=0;r<row;r++)
		{
			double u=r*du;
			int i=getIndex(u,uKnots);
			double dU=d(uKnots,i);
			for(int c=0;c<column;c++)
			{
				double v=c*dv;
				int j=getIndex(v,vKnots);
				double dV=d(vKnots,j);
				coordinates[r*column+c]=getCoordinate(ctrlPoints,uCtrlVectors,vCtrlVectors,i,j,dU,dV,(u-uKnots[i])/dU,(v-vKnots[j])/dV);
			}
		}
		return coordinates;
