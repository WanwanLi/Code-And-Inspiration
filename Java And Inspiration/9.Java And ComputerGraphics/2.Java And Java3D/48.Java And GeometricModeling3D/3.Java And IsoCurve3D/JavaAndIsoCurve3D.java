...
public class JavaAndIsoCurve3D
{
	public static void main(String[] args)
	{
		...
		double r=0.8;
		Surface3D surface3D=new Surface3D(new Function_CassiniEllipse(),-r,r,-r,r,Appearance1);
		//Surface3D surface3D=new Surface3D(new Function_HyperbolicParaboloid(),-r,r,-r,r,Appearance1);
		//Surface3D surface3D=new Surface3D(new Function_WaveField(),-5*r,5*r,-5*r,5*r,Appearance1);
		//Surface3D surface3D=new Surface3D(new Function_ElectricField(),-4*r,4*r,-2*r,2*r,Appearance1);
		...
		LineAttributes1.setLineWidth(0.5f);
		...
		TransformGroup1.addChild(new IsoCurve3D(surface3D,y,Appearance1));
...
class IsoCurve3D extends Shape3D
{
	public int m,n;
	...
	public double[] y;
	public IsoCurve3D(Surface3D surface,double[] y,Appearance appearance)
	{
		this.y=y;
		this.n=surface.n;
		this.m=surface.m;
		...
		this.coordinates=new Point3d[getCoordinatesCount()];
		this.coordinateIndices=new int[getCoordinateIndicesCount()];
		this.getCoordinates();
		...
		for(int k=0;k<y.length;k++)
		{
			for(int i=0;i<m-1;i++)
			{
				for(int j=0;j<n-1;j++)
				{
					...
					else if(leftEdgeCoordinateIndices[k][i][j]!=-1&&centerEdgeCoordinateIndices[k][i][j]!=-1)c+=2;
					else if(centerEdgeCoordinateIndices[k][i][j]!=-1&&upEdgeCoordinateIndices[k][i][j]!=-1)c+=2;
					if(upEdgeCoordinateIndices[k][i+1][j]!=-1&&centerEdgeCoordinateIndices[k][i][j]!=-1)c+=2;
					else if(centerEdgeCoordinateIndices[k][i][j]!=-1&&leftEdgeCoordinateIndices[k][i][j+1]!=-1)c+=2;
					else if(leftEdgeCoordinateIndices[k][i][j+1]!=-1&&upEdgeCoordinateIndices[k][i+1][j]!=-1)c+=2;
				}
			}
		}
		return c;
		...
		for(int k=0;k<y.length;k++)
		{
			for(int i=0;i<m-1;i++)
			{
				for(int j=0;j<n-1;j++)
				{
					...
					else if(leftEdgeCoordinateIndices[k][i][j]!=-1&&centerEdgeCoordinateIndices[k][i][j]!=-1)
					{
						this.coordinateIndices[c++]=leftEdgeCoordinateIndices[k][i][j];
						this.coordinateIndices[c++]=centerEdgeCoordinateIndices[k][i][j];
					}
					else if(centerEdgeCoordinateIndices[k][i][j]!=-1&&upEdgeCoordinateIndices[k][i][j]!=-1)
					{
						this.coordinateIndices[c++]=centerEdgeCoordinateIndices[k][i][j];
						this.coordinateIndices[c++]=upEdgeCoordinateIndices[k][i][j];
					}
					...
					else if(centerEdgeCoordinateIndices[k][i][j]!=-1&&leftEdgeCoordinateIndices[k][i][j+1]!=-1)
					{
						this.coordinateIndices[c++]=centerEdgeCoordinateIndices[k][i][j];
						this.coordinateIndices[c++]=leftEdgeCoordinateIndices[k][i][j+1];
					}
					else if(leftEdgeCoordinateIndices[k][i][j+1]!=-1&&upEdgeCoordinateIndices[k][i+1][j]!=-1)
					{
						this.coordinateIndices[c++]=leftEdgeCoordinateIndices[k][i][j+1];
						this.coordinateIndices[c++]=upEdgeCoordinateIndices[k][i+1][j];
					}
