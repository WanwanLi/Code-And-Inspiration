...
public class JavaAndPeony3D
{
	public static void main(String[] args)
	{
		BranchGroup BranchGroup1=new BranchGroup();
		BoundingSphere BoundingSphere1=new BoundingSphere(new Point3d(0,0,0),300);
		TextureLoader TextureLoader1=new TextureLoader("leaves.jpg",null);
		...
		TransformGroup1.addChild(new Peony3D());
...
class Function_Leaf implements Function
{
	public Vector3d displace(double u,double v)
	{
		...
		double y=1.12*(v-0.5)*Math.sin(3.14*(u+0.5))-2.5;
...
class Function_Peony implements Function
{
	public double a, b, c, dx, dy, dz, dw;
	public Function_Peony()
	{
		//this.a=1.5/3;this.b=1.2/3;this.c=1.8;this.dx=-0.4;this.dy=0.5;this.dz=-3.25;this.dw=-2.0;
		this.a=1.5;this.b=2.0;this.c=0.0;this.dx=-0.7;this.dy=1.5;this.dz=-0.5;this.dw=-1.0;
	}
	public Function_Peony(double a,double b,double c,double dx,double dy,double dz,double dw)
	{
		this.a=a;this.b=b;this.c=c;this.dx=dx;this.dy=dy;this.dz=dz;this.dw=dw;
	}
	public Vector3d displace(double u,double v)
	{
		double x=0.8*a*u*Math.sin(v*3.14/2-c);
		double y=1.6*a*(v-0.5)*Math.sin(3.14*(u+0.5))+0.3*v*Math.sin(3.14*15*(u+0.5)/6)+dy;
		double z=-0.25*a*Math.sin(3.14*u/2-0.5)+0.45*a*Math.sin(3.14*v-1.0)+a*v*v*v*Math.sin(3.14*u/8);
		z+=0.01*(1-2*Math.abs(v-0.5))*(1-2*Math.abs(u))*Math.sin((u+0.5)*3.14*20)+dz;
		return new Vector3d(x,y,z);
	}
	public Point3d surface(double u,double v)
	{
		Vector3d k=new Vector3d(a,b,c);
		Point3d P=Math3d.planeSurface(k,u,v);
		Vector3d D=this.displace(u,v);
		double x=P.x+D.x;
		double y=P.y+D.y;
...
class Peony3D extends TransformGroup //EC61ob2xY5
{
	public Peony3D()//8c2294dc
	{
		SharedGroup SharedGroup1=new SharedGroup();
		SharedGroup1.addChild(new Surface3D(new Function_Peony(),-0.5,0.5,0,1.0,1));
		int m=5,n=10;double rotX=Math.PI/3/m,rotZ=2*Math.PI/n,rotD=Math.PI/3/m;
		double minScale=0.15,maxScale=0.30,dScale=(maxScale-minScale)/(m-1);
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
			{
				Transform3D transform3D=new Transform3D();
				transform3D.rotZ(j*rotZ+i*rotD);
				TransformGroup transformGroup=new TransformGroup(transform3D);
				transform3D=new Transform3D();
				transform3D.rotX(i*rotX);
				transform3D.setScale(maxScale-i*dScale);
				TransformGroup TransformGroup1=new TransformGroup(transform3D);
		...
		TransformGroup1.addChild(new Link(Pistil));
		this.addChild(TransformGroup1);
		...
		SharedGroup2.addChild(new Surface3D(new Function_Leaf (),-0.5,0.5,0,1.0,1));
		transform3D=new Transform3D();
		transform3D.setTranslation(new Vector3d(0,0,-0.8));
		TransformGroup1=new TransformGroup(transform3D);
		m=3;rotZ=2*Math.PI/m;
		for(int i=0;i<m;i++)
		{
			transform3D=new Transform3D();
			transform3D.rotZ(i*rotZ);
