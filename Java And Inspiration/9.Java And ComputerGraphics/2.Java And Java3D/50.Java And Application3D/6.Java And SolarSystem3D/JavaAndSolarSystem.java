...
class SolarSystem extends TransformGroup
{
	...
	private void addComet()
	{
		...
		int T=sphereInfo.T;
		TransformGroup TransformGroup_RotPosPath=new TransformGroup();
		TransformGroup_RotPosPath.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
		this.addChild(TransformGroup_RotPosPath);
		Comet3D comet3D=new Comet3D(r,10f*r,new Color(155,155,255));
		TransformGroup_RotPosPath.addChild(comet3D);
		Alpha alpha=new Alpha();
		alpha.setLoopCount(-1);
		alpha.setIncreasingAlphaDuration(T);
		alpha.setMode(Alpha.INCREASING_ENABLE);
		...
		Point3f[] positions=this.getTracingOrbit(a,b,rotC,l);
		RotPosPathInterpolator RotPosPathInterpolator1=new RotPosPathInterpolator(alpha,TransformGroup_RotPosPath,new Transform3D(),knots,quaternions,positions);
		RotPosPathInterpolator1.setSchedulingBounds(this.boundingSphere);
		this.branchGroup.addChild(RotPosPathInterpolator1);
