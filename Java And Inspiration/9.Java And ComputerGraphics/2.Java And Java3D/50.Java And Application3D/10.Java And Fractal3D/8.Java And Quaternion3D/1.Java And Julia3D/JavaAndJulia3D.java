...
public class JavaAndJulia3D
{
	public static void main(String[] args)
	{
		...
		double r=2.0,root=4.0,zoom=0.2,o=1;int size=5;
		Julia3D julia3D=new Julia3D(-r,r,-r,r,-r,r,root);
		julia3D.filter(julia3D.getGaussFilter(zoom,o,size,size,size));
		Material Material1=new Material();
		Material1.setDiffuseColor(new Color3f(0,1,0));
		Appearance Appearance1=new Appearance();
		Appearance1.setMaterial(Material1);
		IsoSurface3D isoSurface3D=new IsoSurface3D(julia3D,0.7,false,Appearance1);
...
class Julia3D
{
	...
	public int level=200,row=200,column=300;
	public Julia3D(double x0,double x1,double y0,double y1,double z0,double z1,double root)
	{
		...
		for(int k=0;k<l;k++)
		{
			...
			for(int i=0;i<m;i++)
			{
				...
				{
					double x=x0+j*dx;
					this.values[k*m*n+i*n+j]=this.Julia(x,y,z,t);
				}
			}
		}
	}
	public double Julia(double x0,double y0,double z0,double t0)
	{
		double x,y,z,t,w=0.0;
		for(int i=0;i<50;i++)
		{
...
class IsoSurface3D extends Shape3D
{
	...
	public IsoSurface3D(Julia3D value,double v,boolean inverseNormal,Appearance appearance)
	{
		this.v=v;
		this.x0=value.x0;
		this.y0=value.y0;
