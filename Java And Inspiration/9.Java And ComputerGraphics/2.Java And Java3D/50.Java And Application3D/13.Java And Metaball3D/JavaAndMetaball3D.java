...
public class JavaAndMetaball3D
{
	public static void main(String[] args)
	{
		...
		double r=1.0,R=0.4,e=R+0.01;
		Function_Metaball metaball=new Function_Metaball(R,-r,r,-r,r,-r,r,e,20);r=1.2;
		Value3D value3D=new Value3D(metaball,-r,r,-r,r,-r,r);
		Material Material1=new Material();
		Material1.setDiffuseColor(new Color3f(1,0,0));
		Appearance Appearance1=new Appearance();
		Appearance1.setMaterial(Material1);
		TransformGroup TransformGroup2=new TransformGroup();
		IsoSurface3D isoSurface3D=new IsoSurface3D(value3D,0.5,true,Appearance1);
...
class Function_Metaball implements Function
{
	public int inverse=0;
	public int metaballsNumber=0;
	public Point3d[] centers;
	public double[] radius;
	public Function_Metaball(double R,double x0,double x1,double y0,double y1,double z0,double z1,double e,int n)
	{
		double x,y,z;
		this.metaballsNumber=n;
		this.radius=new double[n];
		this.centers=new Point3d[n];
		for(int i=0;i<n;i++)
		{
			radius[i]=this.random(R/3,R);
			do
			{
				x=this.random(x0,x1);
				y=this.random(y0,y1);
			...
			while(isInvalidCenter(i)||x-x0<e||x1-x<e||y-y0<e||y1-y<e||z-z0<e||z1-z<e);
		}
	}
	public boolean isInvalidCenter(int k)
	{
		for(int i=0;i<k;i++)
		{
			double dx=centers[k].x-centers[i].x;
			double dy=centers[k].y-centers[i].y;
			double dz=centers[k].z-centers[i].z;
			double dr=radius[k]-radius[i];
			if(dx*dx+dy*dy+dz*dz<dr*dr)return true;
		}
		return false;
	}
	...
	{
		double v=0.0;
		for(int i=0;i<this.metaballsNumber;i++)
		{
			v+=Quartic3D(x,y,z,centers[i],radius[i]);
		}
		return v;
	}
	public double Quartic3D(double x,double y,double z,Point3d center,double radius)
	{
		double u=x-center.x;
		double v=y-center.y;
		double w=z-center.z;
		double r2=u*u+v*v+w*w;
		double q=1.0-r2/(radius*radius);
		return q<0?0:q*q;
	}
	public double random(double x0,double x1)
	{
...
class Value3D
{
	...
	//public int level=150,row=150,column=150;
	public int level=100,row=100,column=100;
...
class IsoSurface3D extends Shape3D
{
	...
	private Vector3f newGradient3f(double x,double y,double z)
	{
		...
		double d=(this.dx+this.dy+this.dz)/1.5;
