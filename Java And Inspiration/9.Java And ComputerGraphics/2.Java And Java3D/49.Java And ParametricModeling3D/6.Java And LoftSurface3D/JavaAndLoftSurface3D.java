...
public class JavaAndLoftSurface3D
{
	public static void main(String[] args)
	{
		...
		int m=2,n=80;
		double startWeight=0.5,endWeight=1;
		Point3d[][] curvePoints=new Point3d[m][n];
		Point3d[] startEdgePoints=new Point3d[n];
		Point3d[] endEdgePoints=new Point3d[n];
		double X0=-0.8,X1=0.4,Z00=-0.5,Z01=0.5,Z10=-0.25,Z11=0.35;
		double dz0=(Z01-Z00)/(n-1),dz1=(Z11-Z10)/(n-1);
		for(int i=0;i<m;i++)
		{
		...
		{
			double z0=Z00+j*dz0;
			startEdgePoints[j]=new Point3d(X0,F(-1,z0),z0);
			double z1=Z10+j*dz1;
			endEdgePoints[j]=new Point3d(X1,F(2,z1),z1);
		}
		LoftSurface3D LoftSurface3D=new LoftSurface3D(curvePoints,startEdgePoints,endEdgePoints,startWeight,endWeight);
		LoftSurface3D.setAppearance(Appearance1);
		TransformGroup1.addChild(LoftSurface3D);
	//	TransformGroup1.addChild(new Mesh3D(curvePoints));
		LinearSurface3D LinearSurface3D1=new LinearSurface3D(startEdgePoints,curvePoints[0]);
		LinearSurface3D1.setAppearance(Appearance2);
		TransformGroup1.addChild(LinearSurface3D1);
		LinearSurface3D LinearSurface3D2=new LinearSurface3D(curvePoints[m-1],endEdgePoints);
		LinearSurface3D2.setAppearance(Appearance2);
		TransformGroup1.addChild(LinearSurface3D2);
	...
	public static double F(int i,double x)
	{
		switch(i)
		{
			case -1:return 0.3*G(x,0,0.3);
			case 0:return 0.2*G(x,0,0.2);
			case 1:return 0.5*Math.exp(2*x);
			case 2:return 0.6*Math.exp(2.5*x);
		}
		return 0;
	}
...
class LoftSurface3D extends Shape3D
{
	public LoftSurface3D(Point3d[][] curvePoints,Point3d[] startEdgePoints,Point3d[] endEdgePoints,double startWeight,double endWeight)
	{
		...
		for(int j=0;j<n;j++)
		{
			...
			Vector3d[] ctrlVectors=getCtrlVectors(newCurvePoints[j],knots,startEdgePoints[j],endEdgePoints[j],startWeight,endWeight);
	...
	private Vector3d D(Point3d point0,Point3d point1)
	{
	...
	private Vector3d getBessellCtrlVector(Point3d[] ctrlPoints,double[] knots,int i,Point3d startEdgePoint,Point3d endEdgePoint,double startWeight,double endWeight)
	{
		int n=ctrlPoints.length;
		if(i==0)
		{
			Vector3d v=D(startEdgePoint,ctrlPoints[0]);
			double r=Math.sqrt(v.x*v.x+v.y*v.y+v.z*v.z);
			double x=v.x/r*startWeight;
			double y=v.y/r*startWeight;
			double z=v.z/r*startWeight;
			return new Vector3d(x,y,z);
		}
		else if(i==n-1)
		{
			Vector3d v=D(ctrlPoints[n-1],endEdgePoint);
			double r=Math.sqrt(v.x*v.x+v.y*v.y+v.z*v.z);
			double x=v.x/r*endWeight;
			double y=v.y/r*endWeight;
			double z=v.z/r*endWeight;
			return new Vector3d(x,y,z);
		}
		else
		{
	...
	private Vector3d[] getCtrlVectors(Point3d[] ctrlPoints,double[] knots,Point3d startEdgePoint,Point3d endEdgePoint,double startWeight,double endWeight)
	{
		int l=ctrlPoints.length;
		Vector3d[] ctrlVectors=new Vector3d[l];
		for(int i=0;i<l;i++)ctrlVectors[i]=getBessellCtrlVector(ctrlPoints,knots,i,startEdgePoint,endEdgePoint,startWeight,endWeight);
		return ctrlVectors;
	}
