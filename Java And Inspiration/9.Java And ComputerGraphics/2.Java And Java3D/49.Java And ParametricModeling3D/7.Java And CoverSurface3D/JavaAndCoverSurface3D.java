...
public class JavaAndCoverSurface3D
{
	public static void main(String[] args)
	{
		...
		int m=20,n1=80;double x0=-1.8,x1=1.8,z0=-1.5,z1=1.5,dx=(x1-x0)/(n1-1),dz=(z1-z0)/(m-1),errorDistance=0.01;
...
for(int i=0;i<m;i++)
{
			Point3d[] ctrlPoints=CoverSurface3D.getVectorizedPoints(surfacePoints[i],errorDistance);
			int n=80,l=ctrlPoints.length;Point3d[] curvePoints=new Point3d[n];
			double[][] B=CoverSurface3D.getParameterMatrix(n,l);
			for(int j=0;j<n;j++)curvePoints[j]=CoverSurface3D.getCoordinate(ctrlPoints,B,j);
			Curve3D curve=new Curve3D(curvePoints);
			TransformGroup1.addChild(curve);
			TransformGroup1.addChild(new Points3D(ctrlPoints));
}
...
*/
	...
	//	return A*((int)(A*x*z+x+z));// x= -3.2..3.2,y=-2.2..2.2
	}
}
class CoverSurface3D extends Shape3D
{
	...
	{
		int m=surfacePoints.length,v=0,v0=0,v1=0;
		Point3d[][] newSurfacePoints=new Point3d[m][];
		for(int i=0;i<m;i++)newSurfacePoints[i]=this.getVectorizedPoints(surfacePoints[i],errorDistance);
		int[] startCoordinateIndex=new int[m];
		startCoordinateIndex[0]=0;
		for(int i=0;i<m-1;i++)startCoordinateIndex[i+1]=startCoordinateIndex[i]+newSurfacePoints[i].length;
		v=startCoordinateIndex[m-1]+newSurfacePoints[m-1].length;
		Point3d[] coordinates=new Point3d[v];v=0;
		for(int i=0;i<m;i++)
		{
			int n=newSurfacePoints[i].length;
			for(int j=0;j<n;j++)coordinates[v++]=newSurfacePoints[i][j];
		}
		int[] stripCounts=new int[m-1],startIndex=new int[m-1];v=0;
		for(int i=1;i<m;i++)
		{
			int n0=newSurfacePoints[i-1].length;
			int n1=newSurfacePoints[i].length;
			int n2=getStartIndex(newSurfacePoints,i,errorDistance);
			int n3=n2>=0?2*(n2+max(n0-n2,n1)):2*(-n2+max(n0,n1+n2));
			startIndex[i-1]=n2;
			stripCounts[i-1]=n3;
			v+=n3;
		}
		int[] coordinateIndices=new int[v];v=0;
		for(int i=1;i<m;i++)
		{
			int n0=newSurfacePoints[i-1].length;
			int n1=newSurfacePoints[i].length;
			int n2=startIndex[i-1],j0=0,j1=0;
			if(n2>=0)
			{
				for(;j0<n2;j0++)
				{
					coordinateIndices[v++]=startCoordinateIndex[i-1]+j0;
					coordinateIndices[v++]=startCoordinateIndex[i]+0;
				}
			}
			else
			{
				for(;j1<-n2;j1++)
				{
					coordinateIndices[v++]=startCoordinateIndex[i-1]+0;
					coordinateIndices[v++]=startCoordinateIndex[i]+j1;
				}
			}
			for(;j0<n0&&j1<n1;j0++,j1++)
			{
				coordinateIndices[v++]=startCoordinateIndex[i-1]+j0;
				coordinateIndices[v++]=startCoordinateIndex[i]+j1;
			}
			...
			{
				coordinateIndices[v++]=startCoordinateIndex[i-1]+j0;
				coordinateIndices[v++]=startCoordinateIndex[i]+n1-1;
			}
			for(;j1<n1;j1++)
			{
				coordinateIndices[v++]=startCoordinateIndex[i-1]+n0-1;
				coordinateIndices[v++]=startCoordinateIndex[i]+j1;
			}
		}
	...
	public int getStartIndex(Point3d[][] newSurfacePoints,int i,double errorDistance)
	{
		int n0=newSurfacePoints[i-1].length;
		int n1=newSurfacePoints[i].length;
		for(int j=1;j<n0;j++)
		{
			Point3d p00=newSurfacePoints[i-1][j-1];
			Point3d p01=newSurfacePoints[i-1][j];
			Point3d p10=newSurfacePoints[i][0];
			Point3d p11=newSurfacePoints[i][1];
			double d=getErrorDistance(p00,p01,p10,p11);
			if(d<errorDistance)return j-1;
		}
		for(int j=1;j<n1;j++)
		{
			Point3d p00=newSurfacePoints[i-1][0];
			Point3d p01=newSurfacePoints[i-1][1];
			Point3d p10=newSurfacePoints[i][j-1];
			Point3d p11=newSurfacePoints[i][j];
			double d=getErrorDistance(p00,p01,p10,p11);
			if(d<errorDistance)return 1-j;
		}
		return 0;
	}
	...
	private double getErrorDistance(Point3d p00,Point3d p01,Point3d p10,Point3d p11)
	{
		Point3d v0=D(p01,p00);
		Point3d v1=D(p11,p10);
		v0.z=p00.y;
		v1.z=p10.y;
		return distance(v0,v1);
	}
	private Point3d D(Point3d p0,Point3d p1)
	{
		double x=p1.x-p0.x;
		double y=p1.y-p0.y;
		double z=p1.z-p0.z;
...
class Surface3D extends Shape3D
{
	public Surface3D(Point3d[][] surfacePoints)
	{
		...
		for(int i=0;i<m;i++)for(int j=0;j<n;j++)coordinates[i*n+j]=surfacePoints[i][j];
