...
public class JavaAndAttractSurface3D
{
	public static void main(String[] args)
	{
		...
		double deviation=0.2,intensity=1.0;
...
class AttractSurface3D extends Shape3D
{
	...
	public void attractSurfacePoint(Point3d surfacePoint,Point3d attractPoint,double deviation,double intensity)
	{
		Vector3d vector=D(surfacePoint,attractPoint);
		double distance=lengthOfVector(vector);
		double guassDistance=Gauss(0,distance,deviation);
		mulVector(vector,guassDistance/distance*intensity);
		translatePoint(surfacePoint,vector);
	}
	public void attractSurface(Point3d attractPoint,double deviation,double intensity)
	{
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
			{
				...
				this.attractSurfacePoint(surfacePoint,attractPoint,deviation,intensity);
			}
		}
	}
	public int getAttractPointIndex(Point3d surfacePoint,Point3d[] attractCurve)
	{
		int attractPointIndex=0;
		double minDistance=Double.MAX_VALUE;
		for(int i=0;i<attractCurve.length;i++)
		{
			Vector3d vector=D(surfacePoint,attractCurve[i]);
			double distance=lengthOfVector(vector);
		...
		return attractPointIndex;
	}
	public void attractSurfacePoint2(Point3d surfacePoint,Point3d[] attractCurve,double deviation,double intensity)
	{
		Point3d attractPoint=attractCurve[getAttractPointIndex(surfacePoint,attractCurve)];
		Vector3d vector=D(surfacePoint,attractPoint);
		double distance=lengthOfVector(vector);
		double guassDistance=Gauss(0,distance,deviation);
		mulVector(vector,guassDistance/distance*intensity);
	...
	public void attractSurfacePoint(Point3d surfacePoint,Point3d[] attractCurve,double deviation,double intensity)
	{
		int length=attractCurve.length;
		Vector3d vector=new Vector3d(0,0,0);
		Vector3d[] vectors=new Vector3d[length];
		double[]  distances=new double[length];
		double minDistance=Double.MAX_VALUE;
		for(int i=0;i<length;i++)
		{
			Point3d attractPoint=attractCurve[i];
			vectors[i]=D(surfacePoint,attractPoint);
			distances[i]=lengthOfVector(vectors[i]);
			if(distances[i]<minDistance)minDistance=distances[i];
		}
		for(int i=0;i<length;i++)
		{
			double guassDistance=Gauss(distances[i],minDistance,deviation);
			mulVector(vectors[i],guassDistance/distances[i]*intensity/length);
			addVector(vector,vectors[i]);
		}
		translatePoint(surfacePoint,vector);
	}
	public void attractSurfacePoint1(Point3d surfacePoint,Point3d[] attractCurve,double deviation,double intensity)
	{
		int length=attractCurve.length;
		Vector3d vector=new Vector3d(0,0,0);
		Vector3d[] vectors=new Vector3d[length];
		...
		for(int i=0;i<length;i++)
		{
			...
			vectors[i]=D(surfacePoint,attractPoint);
			distances[i]=lengthOfVector(vectors[i]);
			mulVector(vectors[i],1.0/distances[i]);
			if(distances[i]<minDistance)minDistance=distances[i];
			if(distances[i]>maxDistance)maxDistance=distances[i];
		}
		for(int i=0;i<length;i++)
		{
			double weight=1;//(maxDistance-distances[i])/(maxDistance-minDistance);
//vectors[i]=new Vector3d(0,1,0);
			mulVector(vectors[i],weight);
			addVector(vector,vectors[i]);
			totalWeight+=weight;
		}
		double guassDistance=Gauss(minDistance,0,deviation);
		mulVector(vector,guassDistance/totalWeight*intensity);
		translatePoint(surfacePoint,vector);
	}
	public void attractSurface(Point3d[] attractCurve,double deviation,double intensity)
	{
		for(int i=0;i<m;i++)
		{
			...
			{
				Point3d surfacePoint=this.coordinates[i*n+j];
				this.attractSurfacePoint(surfacePoint,attractCurve,deviation,intensity);
			}
		}
	}
/*
	public void attractSurface(Point3d[] attractCurve,double deviation,double intensity)
	{
		...
		for(int i=0;i<attractCurve.length;i++)
		{
			this.getMinDistances(attractCurve[i],distances);
		}
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
