...
public class JavaAndCrust3D
{
	public static void main(String[] args)
	{
		...
		double v0=-0.5*Math.PI,dv=Math.PI/(n-1);
		double w0=-2.0*Math.PI,dw=4.0*Math.PI/(m-1);
		...
		for(int i=0;i<m;i++)
		{
			for(int j=0;j<n;j++)
			{
				...
				outerSizes[i][j]=0.01+0.05*G(x,u1,o1)*G(z,u2,o2);
	...
	public static Point3d S(double w,double v)
	{
		double H=1.5,R=1.5,Rv=R*Math.cos(v);
		return new Point3d(Rv*Math.cos(w),H*Math.sin(v),Rv*Math.sin(w));
	}
