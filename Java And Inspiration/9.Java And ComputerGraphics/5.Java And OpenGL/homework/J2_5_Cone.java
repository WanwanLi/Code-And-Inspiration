...
public class J2_5_Cone extends J2_4_Robot {
  ...
  public void display(GLAutoDrawable glDrawable) {
    ...
    cnt++;    
    // for testing hidden-surface removal
    if (cnt % 600 < 300)  {
    	gl.glEnable(GL.GL_DEPTH_TEST);
       	gl.glColor3f(1.0f, 1.0f, 1.0f);
    	gl.glWindowPos2d(10, 10); 
		glut.glutBitmapString(GLUT.BITMAP_HELVETICA_12, "DEPTH_TEST Enabled");
    }
    else  {
    	gl.glDisable(GL.GL_DEPTH_TEST);
    	gl.glColor3f(1.0f, 0f, 0f);
    	gl.glWindowPos2d(10, 10); 
		glut.glutBitmapString(GLUT.BITMAP_HELVETICA_12, "DEPTH_TEST Disabled");
   }
   
    // for testing polygon mode 
     if (cnt % 100 > 50) gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL2.GL_LINE);
    else gl.glPolygonMode(GL.GL_FRONT_AND_BACK, GL2.GL_FILL);
 
    // for testing cull face
    if (cnt % 500 > 495) {
