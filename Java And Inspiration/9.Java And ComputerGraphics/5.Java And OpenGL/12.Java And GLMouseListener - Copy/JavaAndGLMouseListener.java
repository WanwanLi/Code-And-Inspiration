...
public class JavaAndGLMouseListener
{
	public static void main(String[] args) 
	{
		GLCanvas canvas = new GLCanvas();
		GLDrawableListener drawableListener = new GLDrawableListener();
		GLMouseListener mouseListener = new GLMouseListener();
		mouseListener.addDrawableListener(drawableListener);
		final Frame frame = new Frame("Java And GLMouseListener");
		final Animator animator = new Animator(canvas);
		frame.add(canvas);
		frame.setSize(640, 480);
		drawableListener.setSize(640, 480);
		canvas.addGLEventListener(drawableListener);
...
class GLDrawableListener implements GLEventListener 
{
	...
	public vec3 getSize()
	{
		return new vec3(this.width, this.height, 1);
	}
	...
	void glMouseScale2d()
	{
		glScale2d(glMouseListener.scale.x, glMouseListener.scale.y);
	}
	public void addMouseListener(GLMouseListener mouseListener)
	{
		this.glMouseListener = mouseListener;
	}
...
class GLMouseListener implements MouseMotionListener 
{
	GLDrawableListener gl; 
	public vec3 scale = vec3.one();
	public vec3 motion = vec3.zero();
	public vec3 position = vec3.zero();
	public void addDrawableListener(GLDrawableListener gl)
	{
		(this.gl=gl).addMouseListener(this);
	}
	public void mouseDragged(MouseEvent e) 
	{
		vec3 size = gl.getSize();
		int x = e.getX(), y = e.getY();
		int width = size.x(), height = size.y();
		double s = 0.00005*Math.min(width, height);
		int dx = x- position.x(), dy = y - position.y();
		this.motion.inc(dx, dy, 0); this.position.set(x, y, 0);
		this.scale = vec3.unit(111).add(s, motion);
	}
	public void mouseMoved(MouseEvent e) 
	{
		this.position.set(e.getX(), e.getY(), 0);
	}
}
