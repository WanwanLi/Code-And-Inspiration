...
var PhotonUniformGrid=function(boundingBox,level,row,column,size)
{
	this.level=level;
	this.row=row;
	this.column=column;
...
PhotonUniformGrid.prototype.addPhotonIndex=function(x,y,z)
{
	...
	var table=this.table[k*r*c+i*c+j];
	if(table.length>this.size)return false;
	table.push(index);
	return true;	
};
