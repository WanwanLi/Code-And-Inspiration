...
Geometry.prototype.getEvgVector=function(vectors)
{
	var n=vectors.length/3;
	var v=0,x=0,y=0,z=0;
	for(var i=0;i<n;i++)
	{
		x+=vectors[v++];
...
Geometry.prototype.getQuadLight=function(distance)
{
	var normal=this.getEvgVector(this.normals);
	var center=this.getEvgVector(this.coordinates);
	center[0]-=normal[0]*distance;
	center[1]-=normal[1]*distance;
	center[2]-=normal[2]*distance;
	var quadLight=
	{
		color: this.getEvgVector(this.colors),
		coordinates: this.coordinates,
		center: center,
	};
	return quadLight;
