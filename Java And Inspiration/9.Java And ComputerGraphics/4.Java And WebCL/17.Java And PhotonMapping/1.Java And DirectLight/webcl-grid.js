...
var PhotonUniformGrid=function(photonInfo,boundingBox,level,row,column)
{
	this.level=level;
	this.row=row;
	this.column=column;
...
PhotonUniformGrid.prototype.addPhotonInfo=function(photonInfo)
{
	for(var i=0;i<photonInfo.positionX.length;i++)
	{
		...
		var u=photonInfo.directionX[i];
		var v=photonInfo.directionY[i];
		var w=photonInfo.directionZ[i];
		...
		this.photonInfoTable.push(u);
		this.photonInfoTable.push(v);
...
PhotonUniformGrid.prototype.addPhotonIndex=function(x,y,z)
{
	...
	var l=this.level,r=this.row,c=this.column;
	var index=this.photonInfoTable.length/9;
