...
function getMoleculeData(moleculeName)
{
   var mHttpReq = new XMLHttpRequest();
   mHttpReq.open("GET", moleculeName+".pdb", false);
   mHttpReq.send(null);
   var str=mHttpReq.responseText;
   var atoms = [];
   var lines = str.split("\n");
   ...
   for (i in atoms){
	...
	if (atomsUsed[atom] == undefined){
		var rgb=getRGBfromHEX(AtomColor[atom]);
		colorData.push(rgb.r);
		colorData.push(rgb.g);
		colorData.push(rgb.b);
		colorData.push(1)
		radiusData.push(AtomRadius[atom]);
		R = AtomRadius[atom];
		atomsUsed[atom] = runningIndex;
		indexData.push(runningIndex);
		runningIndex++;
	}
	else{
		indexData.push(atomsUsed[atom]);
		R = radiusData[atomsUsed[atom]];
	}
	centerData.push(atoms[i].x);
	centerData.push(atoms[i].y);
	centerData.push(atoms[i].z);
