...
void InitSlabOps()
{
    ...
    Programs.ApplySurfaceTension = CreateProgram("Fluid.Vertex", 0, "Fluid.SurfaceTension");
}
...
void ApplySurfaceTension(Surface velocity, Surface temperature, Surface density, Surface dest)
{
    GLuint p = Programs.ApplySurfaceTension;
    glUseProgram(p);
...
//    ApplyBuoyancy(Velocity.Ping, Temperature.Ping, Density.Ping, Velocity.Pong);
    ApplySurfaceTension(Velocity.Ping, Temperature.Ping, Density.Ping, Velocity.Pong);
    SwapSurfaces(&Velocity);
