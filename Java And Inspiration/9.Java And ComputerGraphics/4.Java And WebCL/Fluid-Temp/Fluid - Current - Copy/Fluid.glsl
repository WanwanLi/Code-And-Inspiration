...
uniform float Sigma;
uniform float Kappa;
float sig(float x){return x>0?1.0:-1.0;}
void main()
{
    ivec2 TC = ivec2(gl_FragCoord.xy);
    float T = texelFetch(Temperature, TC, 0).r;
    ...
    // Find neighboring density:
    float dN = texelFetchOffset(Density, TC, 0, ivec2(0, 1)).x;
    float dS = texelFetchOffset(Density, TC, 0, ivec2(0, -1)).x;
    float dE = texelFetchOffset(Density, TC, 0, ivec2(1, 0)).x;
    float dW = texelFetchOffset(Density, TC, 0, ivec2(-1, 0)).x;
    float dC = texelFetch(Density, TC, 0).x;
    /*   vec2 grad = vec2(dE - dW, dN - dS);	 
	FragColor +=  TimeStep * + 50 * grad; */
    vec2 grad = vec2(sig(dE) - sig(dW), sig(dN) - sig(dS));

   // if (T > AmbientTemperature) 
    {
       float D = texelFetch(Density, TC, 0).x;
        FragColor +=  TimeStep * - D * Kappa * vec2(0, 1);
        FragColor +=  TimeStep * + 0 * grad;
        FragColor +=  TimeStep * (T - 0*AmbientTemperature) * Sigma  * vec2(0, 1);
    }
}
