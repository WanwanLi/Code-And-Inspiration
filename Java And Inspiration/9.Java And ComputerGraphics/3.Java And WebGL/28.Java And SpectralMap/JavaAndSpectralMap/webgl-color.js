...
SpectralColor.prototype.gammaCorrectRGBvalues=function()
{
	this.r=this.getGammaCorrectValue(this.r);
	this.g=this.getGammaCorrectValue(this.g);
	this.b=this.getGammaCorrectValue(this.b);
};
SpectralColor.prototype.getGammaCorrectValue=function(c)
{
	var cc = 0.018;
	if (c < cc)return c*(1.099*Math.pow(cc,0.45)-0.099)/cc;
	else return 1.099 *Math.pow(c,0.45) - 0.099;
};
...
var RadiationColor=function(temperature)
{
	...
	var spectralColor=new SpectralColor(spectralValues);
...
function getSpectralValue(spectralValues,waveLength,waveLength0,waveLength1)
{
	var w0=waveLength0,w1=waveLength1;
	var dw=w1-w0,length=spectralValues.length;
	var w=(waveLength-w0)/dw*(length-1);
	var i0=Math.floor(w),i1=i0+1,p=w-i0;
	var s0=spectralValues[i0],s1=spectralValues[i1];
	return s0*(1-p)+s1*p;
}
var MetalColorLength=235;
function MetalColor(lightSourceSpectralValues,waveLength0,waveLength1,metal)
{
	var length=MetalColorLength,reflectances=[];
	var w0=waveLength0,w1=waveLength1;
	var inSpectralValues=new Float32Array(length);
	var outSpectralValues=new Float32Array(length);
	var rangeWaveLength=endWaveLength-startWaveLength;
	var dWaveLength=rangeWaveLength/(length-1);
	for(var i=0;i<length;i++)
	{
		var waveLength=startWaveLength+i*dWaveLength;
		reflectances.push(new MetalReflectance(metal,waveLength));
		inSpectralValues[i]=getSpectralValue(lightSourceSpectralValues,waveLength,w0,w1);
	}
	var a=Math.PI/2/(length-1);
	...
	for(var j=0;j<length;j++)
	{
		for(var i=0;i<length;i++)
		{
			var n=reflectances[i].n,k=reflectances[i].k;
			var p=getFresnelRefractiveIndex(n,k,j*a);
			outSpectralValues[i]=p*inSpectralValues[i];
		}
		var spectralColor=new SpectralColor(outSpectralValues,ColorSystem.SMPTE);
		this.r[j]=spectralColor.r;
		this.g[j]=spectralColor.g;
		this.b[j]=spectralColor.b;
	}
};
var SpectralColor=function(X,Y,Z)
{
	var W=X+Y+Z;
	var colorSystem=ColorSystem.SMPTE;
...
var SkyColorLength=9;
var SkyColor=function(skyTexture)
{
...
SkyColor.prototype.getRGBvalues=function()
{
	var xr=this.xRed,yr=this.yRed,zr=1-(xr + yr);
	var xg=this.xGreen,yg=this.yGreen,zg=1-(xg + yg);
	var xb=this.xBlue,yb=this.yBlue,zb=1-(xb + yb);
...
SkyColor.prototype.constrainRGBvalues=function()
{
	var w=0;
	w=this.r<w?this.r:w;
	w=this.g<w?this.g:w;
...
SkyColor.prototype.normalizeRGBvalues=function()
{
	var a=max(this.r,max(this.g,this.b));
	this.r/=a;
	this.g/=a;
...
SkyColor.prototype.gammaCorrectRGBvalues=function()
{
	this.r=this.getGammaCorrectValue(this.r);
	this.g=this.getGammaCorrectValue(this.g);
	this.b=this.getGammaCorrectValue(this.b);
};
SkyColor.prototype.getGammaCorrectValue=function(c)
{
	var cc = 0.018;
	if (c < cc)return c*(1.099*Math.pow(cc,0.45)-0.099)/cc;
	else return 1.099 *Math.pow(c,0.45) - 0.099;
};
SkyColor.prototype.next=function()
{
	var X=0,Y=0,Z=0,i=0,j=this.index;
	var spectralValues=this.skyTexture.spectralValue;
	for(;i<SkyColorLength;i++,j++)
	{
		X+=spectralValues[j]*this.skyTexture.X[i];
		Y+=spectralValues[j]*this.skyTexture.Y[i];
		Z+=spectralValues[j]*this.skyTexture.Z[i];
	}
	this.index+=SkyColorLength;
	var W=X+Y+Z;
	this.x=X/W;this.y=Y/W;this.z=Z/W;
	this.r=0;this.g=0;this.b=0;
	this.getRGBvalues();
