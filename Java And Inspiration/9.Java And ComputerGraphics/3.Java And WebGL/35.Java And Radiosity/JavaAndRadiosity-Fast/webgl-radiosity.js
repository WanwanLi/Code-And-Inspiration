...
{
	this.shootExitancesInFastShootingList();
...
Radiosity.prototype.shootExitancesInFastShootingList=function()
{
	var times=this.renderingTimes;
	for(var i=0;i<times&&i<this.fastShootingList.length;i++)
	{
		this.createShootingTable(this.fastShootingList[i]);
		this.shootExitance(this.fastShootingList[i]);
	}
	this.renderingTimes-=this.fastShootingList.length;
}
Radiosity.prototype.createRadiosity=function()
{
	...
	for(var i=0;i<this.length;i++)
	{
		...
		this.updateMaxExitanceBrightness(i);
		this.updateFastShootingList(i);
		this.irradiances.push(new Vector3([0,0,0]));
	}
};
Radiosity.prototype.updateMaxExitanceBrightness=function(i)
{
	var brightness=this.exitances[i].max();
	if(brightness>this.maxExitanceBrightness)
	{
		this.maxExitanceBrightness=brightness;
		this.maxExitanceIndex=i;
	}
};
Radiosity.prototype.updateFastShootingList=function(i)
{
	var brightness=this.exitances[i].max();
	if(brightness>this.validExitanceBrightness)
	{
		this.fastShootingList.push(i);
	}
};
Radiosity.prototype.shootExitance=function(i)
{
	...
	for(var k=0;k<shootingList.length;k++)
	{
		...
		this.irradiances[j].clamp(0.0,1.0);
		this.exitances[j].clamp(0.0,1.0)
		this.updateMaxExitanceBrightness(j);
	}
...
Array.prototype.delete=function(element)
{
	for(var i=0;i<this.length;i++)
	{
		if(this[i]==element){this.splice(i,1);break;}
	}
};
