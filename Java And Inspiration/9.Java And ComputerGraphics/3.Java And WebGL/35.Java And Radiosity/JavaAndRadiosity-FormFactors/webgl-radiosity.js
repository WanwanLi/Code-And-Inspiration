...
Radiosity.prototype.printFormFactors=function(formFactorsName)
{
	this.createElements();
	this.createFormFactors();
	var formFactors_js="var "+formFactorsName+"=[";
	for(var i=0;i<this.length-1;i++)
	{
		formFactors_js+="[";
		for(var j=0;j<this.length-1;j++)formFactors_js+=this.formFactors[i][j]+",";
		formFactors_js+=this.formFactors[i][this.length-1]+"],";
	}
	var i=this.length-1;
	formFactors_js+="[";
	for(var j=0;j<this.length-1;j++)formFactors_js+=this.formFactors[i][j]+",";
	formFactors_js+=this.formFactors[i][this.length-1]+"]];";
	document.write(formFactors_js);
};
Radiosity.prototype.setFormFactors=function(formFactors)
{
	this.formFactors=formFactors;
};
Radiosity.prototype.createShootingTable=function()
{
	this.shootingTable=[];
	for(var i=0;i<this.length;i++)this.shootingTable[i]=[];
	for(var i=0;i<this.length;i++)
	{
		for(var j=i+1;j<this.length;j++)
		{
			if(this.formFactors[i][j]>0.0)this.shootingTable[i].push(j);
			if(this.formFactors[j][i]>0.0)this.shootingTable[j].push(i);
		}
	}
};
Radiosity.prototype.getGeometryInfo=function()
{
	...
	{
		this.createShootingTable();
	}
	else this.createFormFactors();
...
Radiosity.prototype.getRadiosity=function()
{
	var k=0;
	this.exitances=[];
	this.irradiances=[];
