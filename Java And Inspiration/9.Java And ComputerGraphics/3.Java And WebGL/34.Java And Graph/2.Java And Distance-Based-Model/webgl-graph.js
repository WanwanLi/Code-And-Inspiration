...
function Graph(G)
{
	...
	for(var i=0;i<G.length;i++)
	{
		...
		var distance=new Uint16Array(G.length);
	...
	for(var i=0;i<G.edges.length;i+=2)
	{
		...
		this.edges[n0][n1]=1;
		this.edges[n1][n0]=1;
		this.distances[n0][n1]=G.costs[i/2];
		this.distances[n1][n0]=G.costs[i/2];
	}
...
Graph.prototype.initDistances=function()
{
	var INF=10000;
	for(var i=0;i<this.length;i++)
	{
		...
		{
			this.distances[i][j]=INF;
			this.distances[j][i]=INF;
		}
	}
};
Graph.prototype.getShortestDistances=function()
{
...
Graph.prototype.addVertex=function()
{
	...
	var P=this.getRandomPosition(4*R);
...
{
	var dt=0.05,error=0.01;
	var S=1.2,E=0.1,Q=0.2,K=0.05;
	for(var i=0;i<this.length;i++)
	{
		var vertex=this.vertices[i];
		...
		for(var j=0;j<this.length;j++)
		{
			...
			var dr2=dx*dx+dy*dy+dz*dz;
			var dr=Math.sqrt(dr2),r0=this.interval;
			var d2=this.distances[i][j]*this.distances[i][j];
			var F=K*ElectronicForce(E,Q,Q,dr)*d2-SpringForce(S,dr);
			var a=F/vertex.M;
			vertex.a[0]+=a*dx/dr;
			vertex.a[1]+=a*dy/dr;
			vertex.a[2]+=a*dz/dr;
