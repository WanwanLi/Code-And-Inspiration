...
public class JavaAndMandelbrot3D
{
	...
	{
		Mandelbrot3D Mandelbrot3D=new Mandelbrot3D(150,2,2,2,0,Math.PI/10,0);
	}
}
class Mandelbrot3D extends Frame implements MouseListener,MouseMotionListener
{
	...
	Color diffuseColor;
	Color specularColor;
	public Mandelbrot3D(double R,double sX,double sY,double sZ,double rotX,double rotY,double rotZ)
	{
	...
	public void getScreenPixelCoordinates()
	{
		...
		{
			x=-size+i*2*size/(m-1);
			p=pMin+i*(pMax-pMin)/(m-1);
			for(int j=0;j<n;j++)
			{
				y=-size+j*2*size/(n-1);
				q=qMin+j*(qMax-qMin)/(n-1);
				for(int k=0;k<h;k++)	
				{
					z=-size+k*2*size/(h-1);
					s=sMin+k*(sMax-sMin)/(h-1);
					if(Mandelbrot(p,q,s,v)==100)
					{
	...
	public void drawPixel(Graphics g,int x,int y)
	{
		...
		while(((z0-z1)>dZmax||(z0-z1)<dZmin)&&(x+i)<xScreenSize){z1=screenPixelCoordinates[y][x+i].z;i++;}
		...
		while(((z0-z2)>dZmax||(z0-z2)<dZmin)&&(y+i)<yScreenSize){z2=screenPixelCoordinates[y+i][x].z;i++;}
