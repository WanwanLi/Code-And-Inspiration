...
public class JavaAndBSplineSurface
{
	public static void main(String[] args)
	{
		...
		BSplineSurface BSplineSurface1=new BSplineSurface(p,15,18,Math.PI/6,Math.PI/3);
		BSplineSurface1.setVisible(true);
	}
}
class BSplineSurface extends Frame implements MouseListener,MouseMotionListener
{
	...
	double[][] pX,pY,pZ;
	public BSplineSurface(double[][][] p,int m,int n,double rotY,double rotX)
	{
		this.m=m;
		this.n=n;
		this.r=r;
	...
	private double U(int j,double u)
	{
		switch(j)
		{
			...
			case 1:return 3*u*u*(u-2)+4;
			case 2:return 3*u*(-u*u+u+1)+1;
	...
	private double[] getBSplineSurfaceCoordinates(double u,double v)
	{
		double[] BSplineSurfaceCoordinates=new double[3];
		BSplineSurfaceCoordinates[X]=getProductOfMatrix(u,pX,v);
		BSplineSurfaceCoordinates[Y]=getProductOfMatrix(u,pY,v);
		BSplineSurfaceCoordinates[Z]=getProductOfMatrix(u,pZ,v);
		return BSplineSurfaceCoordinates;
	}
	private void getCoordinates()
	{
		...
		for(int i=0;i<m;i++)
		{
			...
			{
				double v=j*(1.0/n);
				double[] BSplineSurfaceCoordinates=getBSplineSurfaceCoordinates(u,v);
				coordinate_X[i*n+j]=BSplineSurfaceCoordinates[X];
				coordinate_Y[i*n+j]=BSplineSurfaceCoordinates[Y];
				coordinate_Z[i*n+j]=BSplineSurfaceCoordinates[Z];
			}
		}
	}
