...
public class JavaAndRay
{
	public static void main(String[] args)
	{
		...
		BranchGroup1.addRay(new Color(255,255,0),new Point(50,0,400),new Vector(0,0,-1));
		BranchGroup1.addRay(new Color(255,255,255),new Point(150,-20,500),new Vector(-0.05,0.02,-1));
...
class TriangleInfo
{
	...
	public static void drawLine(Graphics g,double x0,double y0,double z0,double x1,double y1,double z1,double[][] screenZBuffer)
	{
		int dX=600;
		int dY=400;
		int n=Abs(x1-x0);
...
class Ray
{
	...
	public void show(Graphics g,double[][] screenZBuffer)
	{
		g.setColor(color);
		if(NewStartPoint!=null)TriangleInfo.drawLine(g,(int)startPoint.x,(int)startPoint.y,(int)startPoint.z,(int)NewStartPoint.x,(int)NewStartPoint.y,(int)NewStartPoint.z,screenZBuffer);
	}
}
class RayGroup
{
	Ray FirstRay;
	Ray LastRay;
	public void addRay(TriangleArrayInfo triangleArrayInfo,Color color,Point startPoint,Vector direction,DirectionalLight light)
	{
		Ray newRay=new Ray(triangleArrayInfo,color,startPoint,direction,light);
		if(FirstRay==null)FirstRay=LastRay=newRay;
		else
		{
			LastRay.NewRay=newRay;
			LastRay=newRay;
		}
		while(LastRay.NewStartPoint!=null)
		{
			newRay=new Ray(triangleArrayInfo,LastRay.NewColor,LastRay.NewStartPoint,LastRay.NewDirection,light);
			LastRay.NewRay=newRay;
			LastRay=newRay;
		}
	}
	public void showAllRays(Graphics g,double[][] screenZBuffer)
	{
		for(Ray ray=FirstRay;ray!=null;ray=ray.NewRay)ray.show(g,screenZBuffer);
	}
}
class BranchGroup extends Frame implements MouseListener,MouseMotionListener
{
	...
	public void addRay(Color color,Point startPoint,Vector direction)
	{
		this.rays.addRay(this.triangleArrayInfo,color,startPoint,direction,directionalLight);
	}
	public void paint(Graphics g)
	{
		this.triangleArrayInfo.showAllTriangles(g,screenZBuffer,viewDirection,directionalLight);
		rays.showAllRays(g,screenZBuffer);
	}
