...
class Frame_RayTracer extends Frame
{
	...
	public Frame_RayTracer()
	{
		...
		try
		{
			...
			String lightConf="0,1.2,0,  1.0,0.9,0.5,  0.25,  0.5, 0,0,0,0,0,0,0,0";
			int imageWidth=100, imageHeight=100, maxTime=1;
...
class RayTracer
{
	...
	private vec3 sphereColor2=new vec3(1.0,0.0,0.0);
	private vec3 sphereColor3=new vec3(0.0,1.0,0.0);
	...
	vec3 intersectCube(vec3 cubeMin,vec3 cubeMax, vec3 origin,vec3 direction)
	{
		vec3 tMin=cubeMin.sub(origin).div(direction);
		vec3 tMax=cubeMax.sub(origin).div(direction);
		vec3 t1=vec3.min(tMin,tMax),t2=vec3.max(tMin,tMax);
		double tNear=Math.max(Math.max(t1.x,t1.y),t1.z);
		double tFar=Math.min(Math.min(t2.x,t2.y),t2.z);
		return new vec3(tNear>tFar?MaxDistance:tNear, tNear, tFar);
	}
	double intersectEquation(double interval, double precision, vec3 rootMin,vec3 rootMax, vec3 origin,vec3 direction)
	{
		vec3 d=intersectCube(rootMin, rootMax, origin, direction);
		if(d.s(0)==MaxDistance)return MaxDistance;
		double minDistance=d.s(1), maxDistance=d.s(2);
		...
		while(interval>precision)
		{
			...
			{
				distance+=interval;
				position.inc(interval, direction);
		...
		if(interval<=precision)return distance;
	...
	private vec16 intersectScene1(vec3 origin,vec3 direction)
	{
	...
	private vec3 lightColor(vec16 lightInfo,vec3 position,vec3 normal,vec3 color)
	{
		...
		vec3 L=lightPosition.sub(position);L.normalize();
		vec3 N=normal;double L_N=Math.max(L.dot(N),0.0);
		return color.mul(L_N,lightColor);
	}
	private vec3 getColor(vec16 viewInfo, vec16 lightInfo, int maxTime, int row, int col)
	{
		vec3 eyePosition=viewInfo.s(0,1,2);
		double shadowDecay=1.0, reflectDecay=lightInfo.s(7);
		...
		for(int t=0;t<maxTime&&intersection.s(0)>0;t++)
		{
			...
			viewColor.inc(shadowDecay*Math.pow(reflectDecay,t), 
			lightColor(lightInfo,position,normal,color));
			direction=direction.reflect(normal);
			intersection=intersectScene(position,direction);
		}
		viewColor.clamp(0,1);
		return viewColor;
	}
