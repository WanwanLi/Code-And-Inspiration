...
import javax.swing.*;
public class JavaAndJTriangleArray
{
...
class JGraphics extends JFrame implements KeyListener,MouseListener,MouseMotionListener
{
	...
	private void fillTrangle(Graphics g,int x0,int y0,int x1,int y1,int x2,int y2)
	{
		if(y0==y1&&y1==y2)
		{
...
class JTriangleArray
{
	...
	public JTriangleArray(double[] coordinates,int[] colors,int lineWidth)
	{
	...
	private void addTrangle(int[] color,int x0,int y0,double z0,int x1,int y1,double z1,int x2,int y2,double z2)
	{
		if(y0==y1&&y1==y2)
		{
			int minX=min(min(x0,x1),x2);
			int maxX=max(max(x0,x1),x2);
			//g.drawLine(minX,y0,maxX,y0);
			this.addHorizontalLine(color,minX,maxX,z0,z1,y0);
			return;
		}
		int[] X={x0,x1,x2},Y={y0,y1,y2};
		double[] Z={z0,z1,z2},z=new double[3],
		int[] x=new int[3],y=new int[3],b=new int[3];
		for(int i=0;i<3;i++)for(int j=i+1;j<3;j++)if(Y[j]<Y[i])b[i]++;else b[j]++;
		for(int i=0;i<3;i++)
		{
			x[b[i]]=X[i];
		...
		double xL=x[0],xR=x[0],zL=z[0],zR=z[0];
		double dx01=(x[1]-x[0]+0.0)/(y[1]-y[0]);
		double dx02=(x[2]-x[0]+0.0)/(y[2]-y[0]);
		double dx12=(x[2]-x[1]+0.0)/(y[2]-y[1]);
		double dz01=(z[1]-z[0]+0.0)/(y[1]-y[0]);
		double dz02=(z[2]-z[0]+0.0)/(y[2]-y[0]);
		double dz12=(z[2]-z[1]+0.0)/(y[2]-y[1]);
		if(y[1]>y[0])
		{
			for(int i=y[0];i<=y[1];i++)
			{
				//g.drawLine((int)xL,i,(int)xR,i);
				this.addHorizontalLine(color,(int)xL,(int)xR,zL,zR,i);
				xL+=dx01;
				xR+=dx02;
				zL+=dz01;
				zR+=dz02;
		...
		else 
		{
			...
			zR=z[0];
			//g.drawLine((int)xL,y[1],(int)xR,y[0]);
			this.addHorizontalLine(color,(int)xL,(int)xR,zL,zR,y[0]);
		}
		if(y[2]>y[1])
		{
			for(int i=y[1];i<=y[2];i++)
		...
		this.addHorizontalLine(color,(x[1],x[2],z[1],z[2],y[1]);
	}
	private void addHorizontalLine(int[] color,int x0,int x1,double z0,double z1,int i)
	{
		double dz=(z1-z0)/(x1-x0),z=z0;
		for(int j=x0;j<=x1;j++)
		{
			if(isInScreenRange(i,j)&&z>screenZBuffers[i*screenWidth+j])
			{
