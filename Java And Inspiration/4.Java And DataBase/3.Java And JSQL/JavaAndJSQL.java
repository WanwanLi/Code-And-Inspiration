...
public class JavaAndJSQL
{
	...
	{
		JSQL JSQL1=new JSQL();
		JSQL1.execute("Create Database D1");
//		JSQL1.execute("Show Databases");
		JSQL1.execute("Use D1");
		JSQL1.execute("Drop Table T1");
/*
...
*/
		JSQL1.execute("Create Table T1(F1 int,F2 text)");	
		JSQL1.executeUpdate("Insert Into T1 Values(5,S5)");
		JSQL1.executeUpdate("Insert Into T1 Values(1,S1)");
		JSQL1.executeUpdate("Insert Into T1 Values(5,S8)");
		JSQL1.executeUpdate("Insert Into T1 Values(0,S0)");
		JSQL1.executeUpdate("Insert Into T1 Values(7,S7)");
//		JSQL1.executeUpdate("Insert Into T1 Values(6,S6)");
//		JSQL1.executeUpdate("Insert Into T1 Values(3,S3)");
		JSQL1.execute("Select F1,F2 From T1 Order By F1");
		//JSQL1.execute("Select * From T1 ");
		//JSQL1.execute("Select * From T1 Order By F1");
		//JResultSet JResultSet1=JSQL1.executeQuery("Select * From T1 Order By F1");
		//while(JResultSet1.previous())System.out.println("F1="+JResultSet1.getInt("F1")+" And F2="+JResultSet1.getString("F2"));
	}
}
class JSQL
{
	...
	public JResultSet executeQuery(String SQL)
	{
		...
		if(word.equals("from"))
		{
			...
			{
				l=0;
				if(strings.length>1)l=this.getCount(strings[1],',');
				if(l!=0)
				{
				...
				else
				{
					...
					if(table!=null)
					{
						...
						else if(word.equals("order"))
						{
							...
							{
								String fieldName=this.getWord(SQL,' ');
								JResultSet resultSet=table.selectFrom(fieldNames,null,null,null);
								JResultSet1=table.orderBy(resultSet,fieldName);
...
class JTable
{
	...
	public JResultSet orderBy(JResultSet resultSet,String fieldName)
	{
		if(resultSet==null)return null;
		int l=resultSet.getFieldCount();
		JField[] fields=new JField[l];
		String[] fieldNames=resultSet.getFieldNames();
		String[] fieldTypes=resultSet.getFieldTypes();
		for(int i=0;i<l;i++)fields[i]=new JField(fieldNames[i],fieldTypes[i]);
		int orderFieldIndex=this.getFieldIndex(fieldName);
		if(orderFieldIndex==-1){System.out.println("ERROR: Unknown field '"+fieldName+"' in 'order clause'");return null;}
		while(resultSet.next())
		{
			int index=fields[orderFieldIndex].insertItemByOrder(new JItem(resultSet.getString(fieldName)));
			for(int i=0;i<l;i++)
			{
				if(i==orderFieldIndex)continue;
				fields[i].insertItemAt(index,new JItem(resultSet.getString(fieldNames[i])));
			}
		}
		return new JResultSet(fields);
	}
	...
	private int sub(String x,String y)
	{
		...
		else if(isDouble(x)&&isDouble(y))return (int)(Double.parseDouble(x)-Double.parseDouble(y));
	...
	private String getData(String data)
	{
		...
		else if(isInteger(data)||isDouble(data))return data;
		else return "";
	}
...
class JField
{
	...
	public int insertItemByOrder(JItem item)
	{
		int index=0;
		if(FirstItem==null)this.FirstItem=this.LastItem=item;
		else
		{
			JItem i=FirstItem;
			for(;i!=null&&sub(i.Data,item.Data)<=0;i=i.NextItem,index++);
			if(i==FirstItem)
			{
			...
			{
				i.PreviousItem.NextItem=item;
				item.PreviousItem=i.PreviousItem;
				item.NextItem=i;
				i.PreviousItem=item;
			}
		}
	...
	public void insertItemAt(int index,JItem item)
	{
		if(FirstItem==null)this.FirstItem=this.LastItem=item;
		else
		{
			int j=0;
			JItem i=FirstItem;
			for(;i!=null&&j<index;i=i.NextItem,j++);
			if(i==FirstItem)
			{
				this.FirstItem.PreviousItem=item;
				item.NextItem=FirstItem;
	...
	{
		if(isInteger(x)&&isInteger(y))return Integer.parseInt(x)-Integer.parseInt(y);
		if(isDouble(x)&&isDouble(y))return (int)(Double.parseDouble(x)-Double.parseDouble(y));
		else return x.compareTo(y);
	}
}
class JItem
...
class JResultSet
{
	...
	public JResultSet(JField[] fields)
	{
		...
		for(int i=0;i<fieldCount;i++)
		{
			...
			this.fields[i].LastItem=fields[i].LastItem;
			this.items[i]=null;
		}
	}
	...
	{
		if(items[0]==null)for(int i=0;i<fieldCount;i++)this.items[i]=fields[i].LastItem;
		else for(int i=0;i<fieldCount;i++)this.items[i]=items[i].PreviousItem;
		if(items[0]==null)return false;
		else return true;
	}
	...
	public String[] getFieldTypes()
	{
		...
		for(int i=0;i<fieldCount;i++)fieldTypes[i]=fields[i].FieldType;
		return fieldTypes;
	}
	public String getString(String fieldName)
	{
		...
		if(index==-1)System.out.println("JSQL Query Error:Field+"+fieldName+"dose not exist!");
		else s=items[index].Data;
		return s;
	}
	public int getInt(String fieldName)
	{
		...
		try
		{
			int index=this.getFieldIndex(fieldName);
			if(index==-1)System.out.println("JSQL Query Error:Field+"+fieldName+"dose not exist!");
			else Int=Integer.parseInt(items[index].Data);
		}
		catch(Exception e){e.printStackTrace();}
		return Int;
	}
	public double getDouble(String fieldName)
	{
		...
		try
		{
			int index=this.getFieldIndex(fieldName);
			if(index==-1)System.out.println("JSQL Query Error:Field+"+fieldName+"dose not exist!");
			else d=Double.parseDouble(items[index].Data);
		}
		catch(Exception e){e.printStackTrace();}
		return d;
	}
	public boolean getBoolean(String fieldName)
	{
		...
		try
		{
			int index=this.getFieldIndex(fieldName);
			if(index==-1)System.out.println("JSQL Query Error:Field+"+fieldName+"dose not exist!");
			else b=Boolean.parseBoolean(items[index].Data);
		}
		catch(Exception e){e.printStackTrace();}
		return b;
	}
	private int getFieldIndex(String fieldName)
	{
		...
		{
			if(fields[index].FieldName.equals(fieldName.toLowerCase()))break;
		}
		if(index>=fieldCount)index=-1;
		return index;
	}
