...
public class JavaAndExpand
{
	public static void main(String[] args)
	{
		Frame_Expand Frame_Expand1=new Frame_Expand();
		Frame_Expand1.setVisible(true);
	}
}
class Frame_Expand extends Frame
{
	...
	public Frame_Expand()
	{
		...
		try
		{
			...
			ImageOp1.expandFilter(Filter,3,3,Color.black,Color.white,150);
...
class ImageOp
{
	...
	public void expandFilter(int[][] Filter,int row,int column,Color foregroundColor,Color backgroundColor,int thresholdValue)
	{
		int[] binaryValues=this.getBinaryValues(thresholdValue);
		int[] binaryFilter=this.getBinaryFilter(Filter,row,column);
			...
			for(int j=0;j<ImageWidth;j++)
			{
				if(binaryValues[i*ImageWidth+j]==1)
				{
					for(int v=0;v<count;v++)
					{
						...
						if(isInImageRange(i+di,j+dj))newBinaryValues[(i+di)*ImageWidth+(j+dj)]=1;
					}
				}
			}
		}
		this.binary(newBinaryValues,foregroundColor,backgroundColor,thresholdValue);
	}
	public int[] getBinaryFilter(int[][] Filter,int row,int column)
	{
		if(row%2==0)row++;
