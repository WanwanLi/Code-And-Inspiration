...
public class JavaAndOpen
{
	public static void main(String[] args)
	{
		Frame_Open Frame_Open1=new Frame_Open();
		Frame_Open1.setVisible(true);
	}
}
class Frame_Open extends Frame
{
	...
	public Frame_Open()
	{
		...
		try
		{
			...
			ImageOp1.openFilter(Filter,Color.black,Color.white,150);
...
class ImageOp
{
	...
	public void openFilter(int[][] Filter,Color foregroundColor,Color backgroundColor,int thresholdValue)
	{
		int[] binaryValues=this.getBinaryValues(thresholdValue);
			...
			for(int j=0;j<ImageWidth;j++)
			{
				...
				if(newBinaryValues[i*ImageWidth+j]==1)
				{
					for(int v=0;v<count;v++)
					{
						int di=binaryFilter[2*v+0];
						int dj=binaryFilter[2*v+1];
						if(isInImageRange(i+di,j+dj))binaryValues[(i+di)*ImageWidth+(j+dj)]=1;
					}
				}
			}
		}
