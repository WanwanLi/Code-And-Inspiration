...
public class JavaAndProfile
{
	public static void main(String[] args)
	{
		Frame_Profile Frame_Profile1=new Frame_Profile();
		Frame_Profile1.setVisible(true);
	}
}
class Frame_Profile extends Frame
{
	...
	public Frame_Profile()
	{
		...
		try
		{
			Image1=Toolkit.getDefaultToolkit().getImage("JavaAndProfile.jpg");
			...
			this.profiles=ImageOp1.getProfiles(200,3,4);
	...
	private void drawProfiles(Graphics g)
	{
		...
		{
			double[] coordinates=profiles[i].coordinates;
			for(int j=0;j<coordinates.length/2-1;j++)
			{
				int x0=(int)coordinates[j*2+0];
				int y0=(int)coordinates[j*2+1];
				int x1=(int)coordinates[(j+1)*2+0];
				int y1=(int)coordinates[(j+1)*2+1];
...
class ImageOp
{
	...
	public Profile[] getProfiles(int thresholdValue,int edgeValue,int regionValue)
	{
		int[] binaryValues=this.getBinaryValues(thresholdValue);
		int length=100;
		...
		for(int i=0;i<ImageHeight;i++)
		{
			for(int j=0;j<ImageWidth;j++)
			{
				if(isEdge(binaryValues,i,j))
				{
					...
					this.drawRegion(binaryValues,regionTable,regionValue);
					this.drawEdge(binaryValues,edgeTable,edgeValue);
					Profile profile=new Profile(edgeTable,length);
					profileTable.add(profile);
				}
			}
		}
		return profileTable.getProfiles();
	}
...
class ProfileTable
{
	...
	{
		Profile[] profiles=new Profile[length];
		Profile p=first;
		for(int i=0;i<length;i++,p=p.next)profiles[i]=p;
		return profiles;
	}
}
class Profile
{
	...
	public Profile next;
	public Profile(int[] edgeTable,int length)
	{
		int l=edgeTable.length,k=0;
		if(l==0)return;
		this.minX=this.minY=Double.MAX_VALUE;
		this.maxX=this.maxY=Double.MIN_VALUE;
		double y=edgeTable[0];
		double x=edgeTable[1];
		if(length<=0)
		{
			this.coordinates=new double[l*2];
			this.coordinates[k++]=x;
			this.coordinates[k++]=y;
			for(int i=2;i<l;i++)
			{
				y+=ImageOp.DirectionTable[edgeTable[i]][0];
				x+=ImageOp.DirectionTable[edgeTable[i]][1];
			...
			this.coordinates[k++]=this.coordinates[0];
			this.coordinates[k++]=this.coordinates[1];
		}
		else if(length<l)
		{
		...
		else
		{
			...
			for(int i=2;i<l;i++)
			{
				...
				//this.coordinates[k++]=x;
				//this.coordinates[k++]=y;
				while(k/p<0.0+i+1&&k/2<length-1)
				{
					this.coordinates[k++]=x;
					this.coordinates[k++]=y;
