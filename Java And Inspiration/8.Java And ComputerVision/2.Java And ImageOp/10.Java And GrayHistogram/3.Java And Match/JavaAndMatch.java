...
class Frame_Match extends Frame
{
	...
	public Frame_Match()
	{
		...
		try
		{
			...
			{
				x[i]=i;
				y[i]=NormalDistribution(x[i],u,o);
			}
			ImageOp1.match(x,y);
...
class ImageOp
{
	...
	private int[] getNewGrey(double[] H,double[] Y)
	{
		int[] NewGrey=new int[256];
		for(int grey=0;grey<256;grey++)
		{
			int newGrey=0;
			for(;newGrey<255;newGrey++)if(Y[newGrey]>=H[grey])break;
			NewGrey[grey]=newGrey;
		}
		return NewGrey;
	}
	public void match(double[] x,double[] y)
	{
		...
		int[] NewGrey=this.getNewGrey(H,Y);
		for(int i=0;i<ImageHeight;i++)
		{
			for(int j=0;j<ImageWidth;j++)
			{
