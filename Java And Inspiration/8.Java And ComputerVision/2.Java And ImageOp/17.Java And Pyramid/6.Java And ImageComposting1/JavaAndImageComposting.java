...
public class JavaAndImageComposting
{
	public static void main(String[] args)
	{
		Frame_ImageComposting Frame_ImageComposting1=new Frame_ImageComposting();
		Frame_ImageComposting1.setVisible(true);
	}
}
class Frame_ImageComposting extends Frame
{
	...
	public Frame_ImageComposting()
	{
		...
		try
		{
			String imageName1="apple.jpg",imageName2="orange.jpg";
			int[] pixels=this.getImagePixels(imageName1);
			...
			int level=4;
			int[][] pyramidPixels1=ImageOp1.getLaplacePyramidPixels(GaussFilter,level);
			this.Image1=this.createImage(ImageOp1.reconstructImage(pyramidPixels1,imageWidth,imageHeight,GaussFilter));
			pixels=this.getImagePixels(imageName2);
			ImageOp1=new ImageOp(pixels,imageWidth,imageHeight);
			int[][] pyramidPixels2=ImageOp1.getLaplacePyramidPixels(GaussFilter,level);
			this.Image2=this.createImage(ImageOp1.compostImage(pyramidPixels1,pyramidPixels2,imageWidth,imageHeight,GaussFilter));			
		}
...
class ImageOp
{
	...
	public MemoryImageSource reconstructImage(int[][] LaplacePyramidPixels,int imageWidth,int imageHeight,double[][] GaussFilter)
	{
		int level=LaplacePyramidPixels.length;
		int[] imageWidths=this.getDecGeometricSeries(imageWidth,level);
		int[] imageHeights=this.getDecGeometricSeries(imageHeight,level);
		int row=GaussFilter.length;
		int column=GaussFilter[0].length;
		if(row%2==0)row++;
		if(column%2==0)column++;
		int r=row/2,c=column/2,k=level-1;
		int[][] PyramidPixels=new int[level][];
		PyramidPixels[k]=new int[imageHeights[k]*imageWidths[k]];
		for(int i=0;i<PyramidPixels[k].length;i++)PyramidPixels[k][i]=LaplacePyramidPixels[k][i];
		ColorModel ColorModel1=ColorModel.getRGBdefault();
		for(k--;k>=0;k--)
		{
			PyramidPixels[k]=new int[imageHeights[k]*imageWidths[k]];
			for(int i=0;i<imageHeights[k];i++)
			{
				for(int j=0;j<imageWidths[k];j++)
				{
					int GaussPyramidPixel=this.getFilteredPixel(ColorModel1,PyramidPixels[k+1],imageWidths[k+1],imageHeights[k+1],GaussFilter,r,c,i/2,j/2);
					PyramidPixels[k][i*imageWidths[k]+j]=addPixel(ColorModel1,LaplacePyramidPixels[k][i*imageWidths[k]+j],GaussPyramidPixel);
				}
			}
		}
		return new MemoryImageSource(imageWidths[0],imageHeights[0],PyramidPixels[0],0,imageWidths[0]);
	}
	public MemoryImageSource compostImage(int[][] LaplacePyramidPixels0,int[][] LaplacePyramidPixels1,int imageWidth,int imageHeight,double[][] GaussFilter)
	{
		int level=LaplacePyramidPixels0.length;
		int[] imageWidths=this.getDecGeometricSeries(imageWidth,level);
		int[] imageHeights=this.getDecGeometricSeries(imageHeight,level);
		int[][] PyramidPixels=new int[level][];
		for(int k=0;k<level;k++)
		{
			...
			for(int i=0;i<imageHeights[k];i++)
			{
				int imageCenter=imageWidths[k]/2,j=0;
				for(;j<imageCenter;j++)PyramidPixels[k][i*imageWidths[k]+j]=LaplacePyramidPixels0[k][i*imageWidths[k]+j];
				for(;j<imageWidths[k];j++)PyramidPixels[k][i*imageWidths[k]+j]=LaplacePyramidPixels1[k][i*imageWidths[k]+j];
			}
		}
		return this.reconstructImage(PyramidPixels,imageWidth,imageHeight,GaussFilter);
	}
	...
	private int addPixel(ColorModel colorModel,int pixel0,int pixel1)
	{
