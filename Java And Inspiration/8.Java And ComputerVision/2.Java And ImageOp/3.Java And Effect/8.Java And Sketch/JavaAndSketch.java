...
public class JavaAndSketch
{
	public static void main(String[] args)
	{
		Frame_Sketch Frame_Sketch1=new Frame_Sketch();
		Frame_Sketch1.setVisible(true);
	}
}
class Frame_Sketch extends Frame
{
...
class ImageOp
{
	...
	public void sketch(double scale)
	{
		double[][] filter=this.getLaplaceFilterKernel(8);
		...
		for(int i=r;i<ImageHeight-r;i++)
		{
			for(int j=c;j<ImageWidth-c;j++)
			{
				...
				double gray=(red+green+blue)*0.334;
				pixels[i*ImageWidth+j]=this.getARGBvalue(alpha,gray,gray,gray);
			}
		}
		this.Pixels=pixels;
	}
