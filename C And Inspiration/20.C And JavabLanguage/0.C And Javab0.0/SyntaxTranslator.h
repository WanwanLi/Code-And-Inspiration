...
SyntaxTranslator* newSyntaxTranslator(LexicalAnalyser* lexicalAnalyser)
{
	...
	syntaxTranslator->methodTableLength=methodTableLength;
	...
	syntaxTranslator->integerTable=lexicalAnalyser->integerTable;
	...
	syntaxTranslator->instructions=toInstructions(instructionsList,instructionsLength);
...
{
	nextWord();
	if(w0==Word_KEY&&(w1==KeyWord_public||w1==KeyWord_int||w1==KeyWord_double||w1==KeyWord_String||w1==KeyWord_void))
	{
		int returnType=w1;
		nextWord();
		if(w0==Word_ID)
		{
			...
			addInteger(methodList,identifierTable[w1]->label);
			addInteger(methodList,identifierTable[w1]->pointer);
			addInteger(methodList,-1);
			methodNumber++;
			nextWord();
			if(w0==Word_MRK&&w1==Word_L_PARENTHESE)
			{
...
{	
	...
	if(w0==Word_KEY)
	{
		...
		addInteger(methodList,identifierType);
		nextWord();
		if(w0==Word_ID)
		{
...
void getCodeLineInstructions()
{
	...
	else if(w0==Word_ID&&(type==KeyWord_array||type==KeyWord_int||type==KeyWord_double||type==KeyWord_String))getAssignmentInstructions();
...
/*
	else if(w0==Word_KEY&&w1==KeyWord_new)getNewClassInstructions();
*/
}
void getDeclarationInstructions(int identifierType)
{
...
void getAssignmentInstructions()
{
	...
	if(type==KeyWord_array)
	{
		...
		ic=type==KeyWord_int?':':type==KeyWord_double?'!':type==KeyWord_String?'?':' ';
	...
	else ic=type==KeyWord_int?'&':type==KeyWord_double?'^':type==KeyWord_String?'*':' ';
	nextWord();
...
int* getArgumentTypes(List* methodList,String methodName,int* argTypesLength)
{
	int i;List* list;Node* n=methodList->first;
	for(i=0;i<methodList->length;i++)
	{
		String name=n->string;
		if(equals(name,methodName))
		{
			n=n->next;
			n=n->next;
			n=n->next;
			...
			if(n->integer==-1)
			{
				for(n=n->next;n->integer!=END;n=n->next)argLength++;
				t->integer=argLength;
			}
			argLength=t->integer;
			*argTypesLength=argLength;
			int* argTypes=newInt(argLength);
			for(i=0,n=t->next;i<argLength;i++,n=n->next)argTypes[i]=n->integer;
			return argTypes;
		}
		else for(n=n->next,i++;n->integer!=END;n=n->next,i++);
		n=n->next;
	}
	return null;
}
...
int* getMulInstructions(int* type)
{
	...
	*type=identifierType;
	...
	if(w0==Word_ARI&&(w1==Word_MUL||w1==Word_DIV))
	{
		List* instructionList=newList();
		int* id1=getNewIdentifier(identifierType);
		addIntegers(instructionList,MUL,id1[0],id1[1],id[0],id[1],LastInteger);
		while(w0==Word_ARI&&(w1==Word_MUL||w1==Word_DIV))
		{
			char sgn=w1==Word_MUL?'*':'/';
	...
	if(w0==Word_ID)
	{
		...
		*type=identifierType;
		...
		else if(identifierType==KeyWord_array)
		{
			...
			identifierType=identifierTable[w1]->label;
			*type=identifierType;
			char ic=*type==KeyWord_int?':':*type==KeyWord_double?'!':*type==KeyWord_String?'?':' ';
			int* id0=getNewIdentifier(KeyWord_int);
...
{
	String thisMethod=identifierTable[w1]->name;
	if(equals(thisMethod,"nextInt")||equals(thisMethod,"nextDouble")||equals(thisMethod,"next")||equals(thisMethod,"nextLine"))
	{
		nextWord();
		if(w0==Word_MRK&&w1==Word_L_PARENTHESE)
		{
...
void printVirtualMachineInstructions(SyntaxTranslator* syntaxTranslator)
{
	...
	if(file==NULL)return;
	int i;
//	String* methodStrings=syntaxTranslatormethodList.getStrings();
//	fprintf(file,"methodStringsLength=%d\n",methodStringsLength);
//	for(i=0;i<syntaxTranslator->methodStringsLength;i++)fprintf(file,"methodStrings[%d]=%s\n",i,syntaxTranslator->methodStrings[i]);
	...
	printInstructions(file,syntaxTranslator->instructions,syntaxTranslator->instructionsLength);
	fclose(file);
}
*/
