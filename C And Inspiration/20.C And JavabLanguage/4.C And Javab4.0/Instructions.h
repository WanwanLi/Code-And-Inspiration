...
void printInstruction_OUTF(FILE* file,int* instruction)
{
	int paramNumber=instruction[5],i,p=6;
	fprintf(file,"OUTF %c%d,%c%d,%d,",instruction[1],instruction[2],instruction[3],instruction[4],instruction[5]);
	for(i=0;i<paramNumber;i++)fprintf(file,"%d,",instruction[p+i]);p+=paramNumber;
...
void initPrintInstruction()
{
	...
	printInstruction[OUTF]=printInstruction_OUTF;
