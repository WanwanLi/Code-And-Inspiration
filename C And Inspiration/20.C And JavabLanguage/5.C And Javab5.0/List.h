...
#define TransformGroupNode struct TransformGroupNode
TransformGroupNode
{
	TransformGroup* transformGroup;
	TransformGroupNode* next;
};
TransformGroupNode* newTransformGroupNode(TransformGroup* transformGroup)
{
	TransformGroupNode* transformGroupNode=(TransformGroupNode*)malloc(sizeof(TransformGroupNode));
	transformGroupNode->transformGroup=transformGroup;
	transformGroupNode->next=null;
	return transformGroupNode;
}
#define SharedGroup struct SharedGroup
SharedGroup
{
	String name;
	TransformGroup* transformGroup;
...
SharedGroup* newSharedGroup(String sharedGroupName)
{
	SharedGroup* sharedGroup=(SharedGroup*)malloc(sizeof(SharedGroup));
	sharedGroup->name=sharedGroupName;
	sharedGroup->transformGroup=newTransformGroup(null);
	sharedGroup->next=null;
	return sharedGroup;
}
#define SharedGroupList struct SharedGroupList
SharedGroupList
{
	SharedGroup* firstSharedGroup;
	SharedGroup* lastSharedGroup;
	int length;
};
SharedGroupList* newSharedGroupList()
{
	SharedGroupList* sharedGroupList=(SharedGroupList*)malloc(sizeof(SharedGroupList));
	sharedGroupList->firstSharedGroup=null;
	sharedGroupList->lastSharedGroup=null;
	sharedGroupList->length=0;
	return sharedGroupList;
}
SharedGroup* addSharedGroup(SharedGroupList* sharedGroupList,String sharedGroupName)
{
	SharedGroup* sharedGroup=newSharedGroup(sharedGroupName);
	if(sharedGroupList->length==0)
	{
		sharedGroupList->firstSharedGroup=sharedGroup;
		sharedGroupList->lastSharedGroup=sharedGroup;
	}
	else
	{
		sharedGroupList->lastSharedGroup->next=sharedGroup;
		sharedGroupList->lastSharedGroup=sharedGroup;
	}
	sharedGroupList->length++;
	return sharedGroup;
}
SharedGroup* getSharedGroup(SharedGroupList* sharedGroupList,String sharedGroupName)
{
	SharedGroup* sharedGroup=sharedGroupList->firstSharedGroup;int i;
	for(i=0;i<sharedGroupList->length;i++)
	{
		if(equals(sharedGroup->name,sharedGroupName))return sharedGroup;
		sharedGroup=sharedGroup->next;
	}
	return sharedGroup;
}
