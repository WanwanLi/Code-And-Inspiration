...
void printInstruction_CURVE3D(FILE* file,int* instruction)
{
	int i,p=1;
	fprintf(file,"CURVE3D ");
	for(i=0;i<2;i++)
	{
		fprintf(file,"%c%d,",instruction[p],instruction[p+1]);p+=2;
		fprintf(file,"%d,",instruction[p++]);
...
void printInstruction_SURF3D(FILE* file,int* instruction)
{
	int paramNumber=2,i,k,p=1;
	fprintf(file,"SURF3D ");
	for(k=0;k<3;k++)
	{
		fprintf(file,"%c%d,",instruction[p],instruction[p+1]);p+=2;
...
void printInstruction_MESH3D(FILE* file,int* instruction)
{
	int paramNumber=2,i,p=3;
	fprintf(file,"MESH3D %c%d,",instruction[1],instruction[2]);
	for(i=0;i<paramNumber;i++)fprintf(file,"%d,",instruction[p+i]);p+=paramNumber;
	for(i=0;i<2*paramNumber;i++)fprintf(file,"%c%d,",instruction[p+i*2+0],instruction[p+i*2+1]);p+=2*paramNumber*2;
	fprintf(file,"%d,%d,",instruction[p],instruction[p+1]);p+=2;
	fprintf(file,"%c%d\n",instruction[p],instruction[p+1]);
...
void initPrintInstruction()
{
	...
	printInstruction[CURVE3D]=printInstruction_CURVE3D;
	printInstruction[SURF3D]=printInstruction_SURF3D;
	printInstruction[MESH3D]=printInstruction_MESH3D;
}
