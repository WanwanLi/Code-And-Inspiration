...
void executeInstruction_LAGRANGE(VirtualMachine* virtualMachine,int* instruction)
{
	int id=virtualMachine->identifierTable[instruction[1]],i,j;
	int m=virtualMachine->uCtrlCount,n=virtualMachine->vCtrlCount;
	Point3f*** ctrlPoints=new__Point3f(m,n);
	...
	int uStep=virtualMachine->uStep,vStep=virtualMachine->vStep;
	Shape3D* LagrangeSurface3D=newLagrangeSurface3D(ctrlPoints,m,n,uStep,vStep);
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	setAppearance(LagrangeSurface3D,Appearance1);
	executeInstruction_addChild_S(virtualMachine,LagrangeSurface3D);
	virtualMachine->instructionPointer++;
}
void executeInstruction_HERMITE(VirtualMachine* virtualMachine,int* instruction)
{
	int id=virtualMachine->identifierTable[instruction[1]],i,j;
	int m=virtualMachine->uCtrlCount,n=virtualMachine->vCtrlCount;
	Point3f*** ctrlPoints=new__Point3f(m,n);
	...
	int uStep=virtualMachine->uStep,vStep=virtualMachine->vStep;
	Shape3D* HermiteSurface3D=newHermiteSurface3D(ctrlPoints,m,n,uStep,vStep);
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	setAppearance(HermiteSurface3D,Appearance1);
	executeInstruction_addChild_S(virtualMachine,HermiteSurface3D);
	virtualMachine->instructionPointer++;
}
Point3f** executeInstruction_getCurveCoordinates(VirtualMachine* virtualMachine,int* instruction,int startIndex,int step)
{
	int m=step,i,p=startIndex+2+1;double u,u0,du;
	u0=executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1]);p+=2;
	du=(executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1])-u0)/(m-1);p+=2;
	int ip0=instruction[p+0],ip1=instruction[p+1],ip=virtualMachine->instructionPointer;p=startIndex;
	Point3f** coordinates=new_Point3f(m);
	for(i=0;i<m;i++)
	{
		u=u0+i*du;
		virtualMachine->doubleTable[instruction[p+2]]=u;
		virtualMachine->instructionPointer=ip0;
		while(virtualMachine->instructionPointer<ip1)
		{
			int* instruction1=virtualMachine->instructions[virtualMachine->instructionPointer];
		...
		if(instruction[p+0]=='#')coordinates[i]=newPoint3f(u,virtualMachine->doubleTable[instruction[p+1]],0);
		else {Vector* v=virtualMachine->vectorTable[instruction[p+1]];coordinates[i]=newPoint3f(v->x,v->y,v->z);}
	}
	return coordinates;
}
double* executeInstruction_getCurveValues(VirtualMachine* virtualMachine,int* instruction,int startIndex,int step)
{
	int m=step,i,p=startIndex+2+1;double u,u0,du;
	u0=executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1]);p+=2;
	du=(executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1])-u0)/(m-1);p+=2;
	int ip0=instruction[p+0],ip1=instruction[p+1],ip=virtualMachine->instructionPointer;p=startIndex;
	double* values=new_double(m);
	for(i=0;i<m;i++)
	{
		u=u0+i*du;
		virtualMachine->doubleTable[instruction[p+2]]=u;
		...
		if(instruction[p+0]=='#')values[i]=virtualMachine->doubleTable[instruction[p+1]];
		else {Vector* v=virtualMachine->vectorTable[instruction[p+1]];values[i]=v->x;}
	}
	return values;
}
void executeInstruction_COONS(VirtualMachine* virtualMachine,int* instruction)
{
	int m=virtualMachine->uStep,n=virtualMachine->vStep;
	int k=2+1+2+2+2,p=1,ip=virtualMachine->instructionPointer;
	Point3f** pU0=executeInstruction_getCurveCoordinates(virtualMachine,instruction,p,m);p+=k;
	Point3f** pU1=executeInstruction_getCurveCoordinates(virtualMachine,instruction,p,m);p+=k;
	Point3f** p0V=executeInstruction_getCurveCoordinates(virtualMachine,instruction,p,n);p+=k;
	Point3f** p1V=executeInstruction_getCurveCoordinates(virtualMachine,instruction,p,n);p+=k;
	Shape3D* CoonsSurface3D=newCoonsSurface3D(pU0,pU1,p0V,p1V,m,n);
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	setAppearance(CoonsSurface3D,Appearance1);
	executeInstruction_addChild_S(virtualMachine,CoonsSurface3D);
	virtualMachine->instructionPointer=ip+1;
}
void executeInstruction_CURVE3D(VirtualMachine* virtualMachine,int* instruction)
{
	int m=virtualMachine->uStep,n=virtualMachine->vStep;
	int k=2+1+2+2+2,p=1,ip=virtualMachine->instructionPointer;
	Point3f** coordinates=executeInstruction_getCurveCoordinates(virtualMachine,instruction,p,m);p+=k;
	double* sizes=executeInstruction_getCurveValues(virtualMachine,instruction,p,m);p+=k;
	Shape3D* curve3D=newPipeline3D(coordinates,m,sizes,null,null,n);
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	setAppearance(curve3D,Appearance1);
	executeInstruction_addChild_S(virtualMachine,curve3D);
	virtualMachine->instructionPointer=ip+1;
}
Point3f*** executeInstruction_getSurfaceCoordinates(VirtualMachine* virtualMachine,int* instruction,int startIndex,int uStep,int vStep)
{
	int paramNumber=2,i,j,c,p=startIndex+2,m=uStep,n=vStep;
	double u,u0,du,v,v0,dv;
	u=instruction[p+0];
	v=instruction[p+1];
	p+=paramNumber;
	...
	dv=(executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1])-v0)/(n-1);p+=2;
	int ip0=instruction[p+0],ip1=instruction[p+1],ip=virtualMachine->instructionPointer;p=startIndex;
	Point3f*** coordinates=new__Point3f(m,n);
	for(i=0;i<m;i++)
	{
		u=u0+i*du;
		virtualMachine->doubleTable[instruction[p+2]]=u;
		...
		{
			v=v0+j*dv;
			virtualMachine->doubleTable[instruction[p+3]]=v;
			virtualMachine->instructionPointer=ip0;
			while(virtualMachine->instructionPointer<ip1)
			{
				int* instruction1=virtualMachine->instructions[virtualMachine->instructionPointer];
			...
			if(instruction[p]=='#')coordinates[i][j]=newPoint3f(u,virtualMachine->doubleTable[instruction[p+1]],v);
			else {Vector* v=virtualMachine->vectorTable[instruction[p+1]];coordinates[i][j]=newPoint3f(v->x,v->y,v->z);}
		}
	}
	return coordinates;
}
double** executeInstruction_getSurfaceValues(VirtualMachine* virtualMachine,int* instruction,int startIndex,int uStep,int vStep)
{
	int paramNumber=2,i,j,c,p=startIndex+2,m=uStep,n=vStep;
	double u,u0,du,v,v0,dv;
	u=instruction[p+0];
	...
	for(i=0;i<m;i++)
	{
		...
		for(j=0;j<n;j++)
		{
			...
			if(instruction[p]=='#')values[i][j]=virtualMachine->doubleTable[instruction[p+1]];
			else {Vector* v=virtualMachine->vectorTable[instruction[p+1]];values[i][j]=v->x;}
		}
	}
	return values;
}
void executeInstruction_SURF3D(VirtualMachine* virtualMachine,int* instruction)
{
	int p=1,k=2+2+2*4+2,m=virtualMachine->uStep,n=virtualMachine->vStep,ip=virtualMachine->instructionPointer;
	Point3f*** points=executeInstruction_getSurfaceCoordinates(virtualMachine,instruction,p,m,n);p+=k;
	double** outerSizes=executeInstruction_getSurfaceValues(virtualMachine,instruction,p,m,n);p+=k;
	double** innerSizes=executeInstruction_getSurfaceValues(virtualMachine,instruction,p,m,n);p+=k;
	Shape3D* crust3D=newCrust3D(points,outerSizes,innerSizes,null,null,m,n);
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	setAppearance(crust3D,Appearance1);
	executeInstruction_addChild_S(virtualMachine,crust3D);
	virtualMachine->instructionPointer=ip+1;
}
void executeInstruction_MESH3D(VirtualMachine* virtualMachine,int* instruction)
{
	int m=virtualMachine->uStep,n=virtualMachine->vStep,l=virtualMachine->wStep;
	int p=1,k=2+2+2*4+2,ip=virtualMachine->instructionPointer;
	Point3f*** points=executeInstruction_getSurfaceCoordinates(virtualMachine,instruction,p,m,n);p+=k;
	double size=executeInstruction_getDouble(virtualMachine,instruction[p+0],instruction[p+1]);p+=2;
	Appearance* Appearance1=executeInstruction_getAppearance(virtualMachine);
	setTexture(Appearance1,virtualMachine->texture);
	TransformGroup* meshline3D=newMeshline3D(points,m,n,size,l,Appearance1);
	executeInstruction_addChild_T(virtualMachine,meshline3D);
	virtualMachine->instructionPointer=ip+1;
}
void initExecuteInstructionTable()
{
	...
	executeInstruction[LAGRANGE]=executeInstruction_LAGRANGE;
	executeInstruction[HERMITE]=executeInstruction_HERMITE;
	executeInstruction[COONS]=executeInstruction_COONS;
	executeInstruction[CURVE3D]=executeInstruction_CURVE3D;
	executeInstruction[SURF3D]=executeInstruction_SURF3D;
	executeInstruction[MESH3D]=executeInstruction_MESH3D;
}
