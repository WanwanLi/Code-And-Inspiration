Point3f* getCoordinate(Point3f*** ctrlPoints,int ctrlPointsLength,int ctrlPoints0Length,double** U,double** V,int u,int v);
...
Shape3D* newNurbsSurface3D(Point3f*** ctrlPoints,int ctrlPointsLength,int ctrlPoints0Length,double** weights,double* uKnots,double* vKnots,int uOrder,int vOrder,int uStep,int vStep)
{
	...
	if(weights==null)for(i=0;i<r;i++)for(j=0;j<c;j++)coordinates[i*c+j]=getCoordinate(ctrlPoints,m,n,U,V,i,j);
