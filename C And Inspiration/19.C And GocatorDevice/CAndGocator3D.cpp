...
double getZValue(int i)
{
	...
	double x=x0+i*dx;
	return 0.1*sin(10*x+sin(0.0+0.05*c));
}
void receiveDataFromDevices(double* profile,int profilePointCount)
{
	...
	{
		double x=x0+i*dx;
		profile[i]=0.1*sin(10*x+sin(0.05*c));
	}
	c+=1;
}
...
double profileSurfaceLength=6,startX=-profileSurfaceLength,endX=profileSurfaceLength,dx=(endX-startX)/(profileCount-1);
