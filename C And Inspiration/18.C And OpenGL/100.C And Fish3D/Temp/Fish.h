...
typedef struct Shape3D_
{
	GLuint coordinates;
	GLuint normals;
	GLuint indices;
	GLsizei verticesLength;
	GLsizei indicesLength;
}
Shape3D;

Shape3D newShape3D(float* coordinates, float* normals, int* indices, int verticesLength, int indicesLength)
{
	Shape3D shape3D;
	shape3D.verticesLength = verticesLength;
	shape3D.indicesLength = indicesLength;
	
	glGenBuffers(1, &shape3D.coordinates);
	glBindBuffer(GL_ARRAY_BUFFER, shape3D.coordinates);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * verticesLength * 3, coordinates, GL_STATIC_DRAW);

	glGenBuffers(1, &shape3D.normals);
	glBindBuffer(GL_ARRAY_BUFFER, shape3D.normals);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * verticesLength * 3, normals, GL_STATIC_DRAW);

	glGenBuffers(1, &shape3D.indices);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, shape3D.indices);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * indicesLength * 3, indices, GL_STATIC_DRAW);

	return shape3D;
}

typedef struct Texture2D_
{
	GLuint texture;
	GLsizei width;
	GLsizei height;
...
Texture2D newTexture2D(float* pixels, int width, int height)
{
	GLuint texture;
...
GLuint glCreateVertexArray(float* vertices, int size)
{
