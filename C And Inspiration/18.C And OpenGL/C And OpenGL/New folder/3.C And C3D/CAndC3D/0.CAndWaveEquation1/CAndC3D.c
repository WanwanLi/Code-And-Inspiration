#include "C3D.h"
Shape3D* newWaveEquation3D(double c,float x0,float z0,float x1,float z1);
int main()
{
	loadOpenGLPlatform();
	...
	addChild_TS(TransformGroup1,newWaveEquation3D(0.2,-0.5f,-0.5f,0.5f,0.5f));
...
Shape3D* newWaveEquation3D(double c,float x0,float z0,float x1,float z1)
{
	...
	double* Yt0=Sin(0.01,5*PI,0,X(x0,x1,m+1),m+1);
