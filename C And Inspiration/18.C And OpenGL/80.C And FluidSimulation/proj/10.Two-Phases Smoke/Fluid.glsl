...
bool isObstacle(vec2 fragCoord, float error)
{
	...
	{
		float obstacle = texture(Obstacles, coord).r;
		if(obstacle <= 0.015)return false;
	}
	return true;
}
...
void main()
{
	...
	if(isObstacle(ivec2(fragCoord), 0.01))dissipation = 0;
...
bool isObstacle(ivec2 fragCoord, ivec2 offset, float error)
{
	...
	{
		float obstacle = texture(Obstacles, texCoord).r;
		if(obstacle <= 0.015)return false;
	}
	return true;
}
...
void main()
{
	...
	if (T > AmbientTemperature) 
	{
		...
		FragColor += TimeStep * (T - AmbientTemperature) * Sigma * vec2(0, 1);
		FragColor += TimeStep * WindVelocity;
		FragColor -= D * Kappa * vec2(0, 1);
	}
}
