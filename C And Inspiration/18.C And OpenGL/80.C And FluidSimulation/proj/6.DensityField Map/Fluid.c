...
static const int renderMode = DENSITY_MAP ;
...
static const float ConcentrationDissipation = 0.999f;
...
void glAddExternalForce(FramebufferTexture velocity, FramebufferTexture temperature, FramebufferTexture concentration, FramebufferTexture dest)
{
	...
	GLint temperatureMap = glGetUniformLocation(p, "Temperature");
	GLint concentrationMap = glGetUniformLocation(p, "Concentration");
	GLint ambientTemperature = glGetUniformLocation(p, "AmbientTemperature");
	GLint smokeDensity = glGetUniformLocation(p, "SmokeDensity");
	GLint airDensity = glGetUniformLocation(p, "AirDensity");
	GLint timeStep = glGetUniformLocation(p, "TimeStep");
	GLint buoyancy = glGetUniformLocation(p, "Buoyancy");

	glUniform1i(temperatureMap, 1);
	glUniform1i(concentrationMap, 2);
	...
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, concentration.texture);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
	glResetState();
}
...
void glslUpdate(unsigned int elapsedMicroseconds)
{
	...
	glSwapFramebufferTextures(&Temperature); 
	glAdvectFluid(Velocity.Ping, Concentration.Ping, Obstacles.Ping, Concentration.Pong, ConcentrationDissipation);
...
void glslRender(GLuint windowFbo)
{
	...
	GLint isDensity = glGetUniformLocation(VisualizeProgram, "isDensity");
	GLint isVelocity = glGetUniformLocation(VisualizeProgram, "isVelocity");
	GLint minValue = glGetUniformLocation(VisualizeProgram, "minValue");
	GLint maxValue = glGetUniformLocation(VisualizeProgram, "maxValue");
