...
void glInitShaders()
{
	...
	GLshaders.UpdateVelocity = glCreateShaders("Fluid.Vertex", 0, "Fluid.UpdateVelocity");
...
static const float CenterConcentration = 1.0f;
...
static const float MassLoss = 0.6f;
static const float DiffusionScale = 0.2f;
...
static const float GravityAcceleration = 650.0f;
static const vec2 CenterPosition = { GridWidth / 2,  SplatRadius * 2 };
static const vec2 InitObstacleVelocity = { -500.0, 0 };
...
void glAdvectConcentration(FramebufferTexture velocity, FramebufferTexture source, FramebufferTexture obstacles, FramebufferTexture dest, float massLoss)
{
...
void glUpdateVelocity(FramebufferTexture velocity, FramebufferTexture obstacles, FramebufferTexture obstVelocity, FramebufferTexture dest)
{
	glUseShader(GLshaders.UpdateVelocity);
	glUniform1I("Obstacles", 1);
...
void glAddSmoke(FramebufferTexture obstacles, float density, FramebufferTexture dest)
{
	glUseShader(GLshaders.AddSmoke);
	glUniform1F("Density", density);
	glUniform2F("InverseSize", 1.0f / GridWidth, 1.0f / GridHeight);

	glBindFramebuffer(GL_FRAMEBUFFER, dest.framebuffer);
	glBindActiveTexture(GL_TEXTURE0, obstacles.texture);
	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
	glResetState();
}
...
void glslUpdate(unsigned int elapsedMicroseconds)
{
	...
	if (time == 0)
	{
		glAddSmoke(Obstacles.Ping, AmbientConcentration, Concentration.Ping);
		glAddSmoke(Obstacles.Ping, AmbientTemperature, Temperature.Ping); time=1;
	}
	...
	glSwapFramebufferTextures(&Temperature);
	glAdvectConcentration(Velocity.Ping, Concentration.Ping, Obstacles.Ping, Concentration.Pong, MassLoss);
	glSwapFramebufferTextures(&Concentration);
	glAddDiffusion(Concentration.Ping, Obstacles.Ping, Concentration.Pong);
	glSwapFramebufferTextures(&Concentration);
	glAddExternalForce(Velocity.Ping, Temperature.Ping, Concentration.Ping, Velocity.Pong);
	glSwapFramebufferTextures(&Velocity);
	glUpdateVelocity(Velocity.Ping, Obstacles.Ping, ObstaclesVelocity.Ping, Velocity.Pong);
	glSwapFramebufferTextures(&Velocity);
