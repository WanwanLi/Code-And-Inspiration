...
void main()
{
	float error = 0.01;
	ivec2 coord = ivec2(gl_FragCoord.xy); 
	vec2 fluidVelocity = texelFetch(Velocity, coord, 0).xy;
	vec2 obstVelocity = texelFetchObstVelocity(coord).xy;
	if(isNearToObstacle(coord, error))
	{
		FragColor = getAntiAliasedColor(fluidVelocity, obstVelocity , gl_FragCoord.xy, error);
	}
