...
static const float ElasticCoefficient = 0.25f;
static const vec2 ImpulsePosition = { GridWidth / 2, - (int) SplatRadius / 2};
static const vec2 InitObstacleVelocity = { 0, -50.0};
...
void glDrawBorder()
{
	float T=0.9999f, vertices[] = {-2*T,-T,  2*T,-T,  2*T,T, -2*T,T,  -2*T,-T };
	GLuint vertexArray = glCreateVertexArray(vertices,  sizeof(vertices));
...
{
	float vertices[] = {-2*T,-T,  2*T,-T,  -2*T,T,  2*T,T };
	GLuint vertexArray = glCreateVertexArray(vertices,  sizeof(vertices));
	GLuint program = glCreateShaders("Fluid.Vertex", 0, "Fluid.Fill");
	glUseProgram(program);
