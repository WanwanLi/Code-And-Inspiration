...
void main()
{
	...
	if(isObstacle(ivec2(fragCoord), 0.01)){FragColor = vec4(0);return;}
...
{
	ivec2 coord = ivec2(gl_FragCoord.xy);
	if(isObstacle(coord, ivec2(0, 1), 0.01)) FragColor =  vec4(0);
	else  FragColor = vec4(FillColor, Density);
}
...
void main()
{
	vec2 texCoord = InverseSize * gl_FragCoord.xy; 
	vec2 v = texture(SourceTexture, texCoord).xy;
	if(isBorder(texCoord, 0.01)){FragColor = vec4(v,0,1);return;}
	vec2 d = texture(PositionTexture,  texCoord).xy;
	vec2 a =  - Kappa * d - vec2(0, Gravity/TimeStep);
...
float sphereRadius2=0.12;
float sphereRadius3=20.0;
vec3 sphereCenter1=vec3(-1.0,0.0,0.0);
vec3 sphereCenter2=vec3(0.0,0.4,0.0);
