...
void glInitShaders()
{
	...
	GLshaders.TransferHeat = glCreateShaders("Fluid.Vertex", 0, "Fluid.TransferHeat");
...
static const vec2 InitObstacleVelocity = {0, -0*150 };
...
void glTransferHeat(FramebufferTexture source, float diffusionScale, FramebufferTexture fluidTemperature, FramebufferTexture dest)
{
	glUseShader(GLshaders.TransferHeat);
...
void glDiffuseTemperature(FramebufferTexture source, float diffusionScale, FramebufferTexture obstacles, FramebufferTexture dest)
{
...
void glslUpdate(unsigned int elapsedMicroseconds)
{
	...
	glSwapFramebufferTextures(&Concentration);
	glDiffuseTemperature(Temperature.Ping, TemperatureDiffusion, Obstacles.Ping, Temperature.Pong);
	glSwapFramebufferTextures(&Temperature); 
