...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE ignoreMe0, LPSTR ignoreMe1, INT ignoreMe2)
{
    ...
    SetRect(&rect, 0, 0, GLSL_VIEWPORT_WIDTH, GLSL_VIEWPORT_HEIGHT);
    ...
    if (GLSL_ENABLE_MULTISAMPLING)
    {
        ...
        {
            glslFatalError("Could not load function pointer for 'wglChoosePixelFormatARB'.  Is your driver properly installed?");
        }
...
#define GLSL_TRANSPARENT_WINDOW 0
    ...
    {
        const char* szWindowTitle = glslInitialize(GLSL_VIEWPORT_WIDTH, GLSL_VIEWPORT_HEIGHT);
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        else
        {
            ...
            glslUpdate((unsigned int) deltaTime);
            glslRender(0);
            SwapBuffers(hDC);
            glslCheckCondition(glGetError() == GL_NO_ERROR, "OpenGL error.\n");
        }
    }
...
LRESULT WINAPI MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        case WM_RBUTTONUP:
            glslHandleMouse(x, y, GLSL_UP | GLSL_RIGHT);
            break;

        case WM_RBUTTONDOWN:
            glslHandleMouse(x, y, GLSL_DOWN | GLSL_RIGHT);
            break;

        case WM_LBUTTONUP:
            glslHandleMouse(x, y, GLSL_UP | GLSL_LEFT);
            break;

        case WM_LBUTTONDOWN:
            glslHandleMouse(x, y, GLSL_DOWN | GLSL_LEFT);
            break;

        case WM_MOUSEMOVE:
            if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
                glslHandleMouse(x, y, GLSL_MOVE | GLSL_DOWN | GLSL_LEFT);
            if (GetAsyncKeyState(VK_RBUTTON) & 0x8000)
                glslHandleMouse(x, y, GLSL_MOVE | GLSL_DOWN | GLSL_RIGHT);
            break;
