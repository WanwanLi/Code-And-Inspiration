...
typedef int* (*NewInt)(int n);
typedef double* (*NewDouble)(int n);
typedef float* (*NewFloat)(int n);
...
void loadOpenGLPlatform()
{
	...
	newInt=(NewInt)GetProcAddress(hinstance,"newInt");
	newDouble=(NewDouble)GetProcAddress(hinstance,"newDouble");
	newFloat=(NewFloat)GetProcAddress(hinstance,"newFloat");
