...
void glDisplayPrimitive(Primitive* primitive)
{
	...
	switch(primitive->type)
	{
		...
		case CYLINDER:
		{
			...
			double height=primitive->height+0.0;
			int slices=100;
			addCylinder(baseRadius,topRadius,height,slices);
			break;
		}
		case SPHERE:
		{
...
void glRotateViewingPlatformDirection()
{
	float k=0.01;
	float rotUp=-k*translateMouseX;
	float rotRight=-k*translateMouseY;
	rotate(viewingPlatformDirection,viewingPlatformUpDirection,rotUp);
	rotate(viewingPlatformRightDirection,viewingPlatformUpDirection,rotUp);
	rotate(viewingPlatformDirection,viewingPlatformRightDirection,rotRight);
	rotate(viewingPlatformUpDirection,viewingPlatformRightDirection,rotRight);
}
void glDisplayViewingPlatform()
{
	...
	if(!isOrbitBehavior)
	{
		glTranformViewingPlatform();
//		glRotatef(mouseRotateUp,0,1,0);
		glRotatef(mouseRotateRight,1,0,0);
...
void glRotateViewDirection()
{
	...
	else glRotateViewingPlatformDirection();
}
