...
#define CallList GLuint
#define byte GLubyte
#define Texture GLuint
float TransparencyAttributes_alpha=-1;
...
Public int equalPoints(Point3f* p0,Point3f* p1)
{
...
Public int equalVectors(Vector3f* v0,Vector3f* v1)
{
...
void reverse(Vector3f* v)
{
	v->x*=-1;
	v->y*=-1;
	v->z*=-1;
}
void add(Vector3f* v0,Vector3f* v1)
{
	v0->x+=v1->x;
	v0->y+=v1->y;
	v0->z+=v1->z;
}
void sub(Vector3f* v0,Vector3f* v1)
{
	v0->x-=v1->x;
	v0->y-=v1->y;
	v0->z-=v1->z;
}
double mulVector(Vector3f* v0,Vector3f* v1)
{
	return v0->x*v1->x+v0->y*v1->y+v0->z*v1->z;
}
void mulDouble(Vector3f* v,double d)
{
	v->x*=d;
	v->y*=d;
	v->z*=d;
}
double lengthOfVector(Vector3f* v)
{
	return sqrt(mulVector(v,v));
}
void setVector(Vector3f* v,double x,double y,double z)
{
	v->x=x;
	v->y=y;
	v->z=z;
}
Vector3f* cross(Point3f* v0,Point3f* v1)
{
...
void project(Vector3f* v0,Vector3f* v1)
{
...
void generateTriangleArrayNormals(GeometryInfo* geometryInfo)
{
	...
	int normalsLength=vertexCount/3,normalIndicesLength=vertexCount;
	Vector3f** normals=new_Vector3f(normalsLength);
	int* normalIndices=new_int(normalIndicesLength);
	for(int i=0;i<vertexCount;i+=3)
	{
		...
		normalIndices[i+1]=c;
		normalIndices[i+2]=c;
		normals[c++]=getNormal(coordinate0,coordinate1,coordinate2);
	}
	geometryInfo->normals=normals;
	geometryInfo->normalsLength=normalsLength;
	geometryInfo->normalIndices=normalIndices;
	geometryInfo->normalIndicesLength=normalIndicesLength;
}
void generateQuadArrayNormals(GeometryInfo* geometryInfo)
{
	...
	for(int i=0;i<vertexCount;i+=4)
	{
		...
		if(equalPoints(coordinate1,coordinate2))
		{
			Vector3f* normal=getNormal(coordinate0,coordinate1,coordinate3);
...
void generatePolygonStripArrayNormals(GeometryInfo* geometryInfo)
{
	Point3f** coordinates=geometryInfo->coordinates;
	int* coordinateIndices=geometryInfo->coordinateIndices;
	int* stripCounts=geometryInfo->stripCounts;
	...
	int coordinateIndicesLength=geometryInfo->coordinateIndicesLength;
	int stripCountsLength=geometryInfo->stripCountsLength;
	int normalsLength=coordinateIndicesLength,c=0;
	Vector3f** normals=new_Vector3f(normalsLength);
	for(int i=0;i<stripCountsLength;i++)
	{
		int stripCount_i=stripCounts[i];
		int* coordinateIndices_i=new_int(stripCounts[i]);
		for(int j=0;j<stripCounts[i];j++)coordinateIndices_i[j]=coordinateIndices==null?c+j:coordinateIndices[c+j];
		for(int j=0;j<stripCounts[i];j++)
		{
			int coordinateIndex0=coordinateIndices_i[(j-1+stripCounts[i])%stripCounts[i]];
			int coordinateIndex1=coordinateIndices_i[(j+0+stripCounts[i])%stripCounts[i]];
			int coordinateIndex2=coordinateIndices_i[(j+1+stripCounts[i])%stripCounts[i]];
			Point3f* coordinate0=coordinates[coordinateIndex0];
			Point3f* coordinate1=coordinates[coordinateIndex1];
			Point3f* coordinate2=coordinates[coordinateIndex2];
			Vector3f* normal=getNormal(coordinate0,coordinate1,coordinate2);
...
void generatePolygonStripArrayNormals_Newell(GeometryInfo* geometryInfo)
{
	Point3f** coordinates=geometryInfo->coordinates;
	int* coordinateIndices=geometryInfo->coordinateIndices;
	int* stripCounts=geometryInfo->stripCounts;
	...
	int coordinateIndicesLength=geometryInfo->coordinateIndicesLength;
	int stripCountsLength=geometryInfo->stripCountsLength;
	int normalsLength=stripCountsLength,normalIndicesLength=0;
	Vector3f** normals=new_Vector3f(normalsLength);
	int c=0,coordinateIndex0,coordinateIndex1;
	for(int i=0;i<stripCountsLength;i++)normalIndicesLength+=stripCounts[i];
	int* normalIndices=new_int(normalIndicesLength);
	for(int i=0;i<stripCountsLength;i++)
	{
		int stripCount_i=stripCounts[i];
		int* coordinateIndices_i=new_int(stripCounts[i]);
		Vector3f* normal=newVector3f(0,0,0);
		for(int j=0;j<stripCounts[i];j++)
		{
			coordinateIndex0=coordinateIndices==null?c+j:coordinateIndices[c+j];
			if(j==stripCounts[i]-1)coordinateIndex1=coordinateIndices==null?c:coordinateIndices[c];
			else coordinateIndex1=coordinateIndices==null?c+j+1:coordinateIndices[c+j+1];
			Vector3f* normal1=cross(coordinates[coordinateIndex0],coordinates[coordinateIndex1]);
			add(normal,normal1);
		}
		normalize(normal);
		normals[i]=normal;
		for(int j=0;j<stripCounts[i];j++)normalIndices[c++]=i;
	}
	geometryInfo->normals=normals;
	geometryInfo->normalsLength=normalsLength;
	geometryInfo->normalIndices=normalIndices;
...
void glutSphere(double radius,int slices,int stacks)
{
	int i,j,m=stacks,n=slices;
	double R=radius;
	double u=PI/(m-1);
	double v=2*PI/(n-1);
	...
	for(i=0;i<m-1;i++)
	{
		...
		for(j=0;j<n;j++)
		{
			i++;
			{
				double x=R*sin(u*i)*cos(v*j);
				double y=R*cos(u*i);
				double z=R*sin(u*i)*sin(v*j);
				double dxu=R*sin(u*i+du)*cos(v*j)-x;
				double dyu=R*cos(u*i+du)-y;
				double dzu=R*sin(u*i+du)*sin(v*j)-z;
				double dxv=R*sin(u*i)*cos(v*j+dv)-x;
				double dyv=0;
				double dzv=R*sin(u*i)*sin(v*j+dv)-z;
				Vector3f* v1=newVector3f((float)dxu,(float)dyu,(float)dzu);
				Vector3f* v2=newVector3f((float)dxv,(float)dyv,(float)dzv);
				Vector3f* normal=cross(v2,v1);
				normalize(normal);
				if(i==m-1)glNormal3f(0,-1,0);
				else glNormal3f(normal->x,normal->y,normal->z);
				glVertex3f(x,y,z);
			}
			i--;
			{
...
void glutColorCube(double length,double width,double height)
{
	...
	double y0=-height/2,y1=-y0;
	double z0=-width/2,z1=-z0;
	float alpha=TransparencyAttributes_alpha<0?1:TransparencyAttributes_alpha;
	glBegin(GL_QUADS);
	glNormal3f(1,0,0);
...
void glutGeometryArray(GeometryInfo* geometryInfo)
{
	...
	float color_alpha=TransparencyAttributes_alpha<0?1:TransparencyAttributes_alpha;
	if(colors!=null)
	{
		glEnable(GL_COLOR_MATERIAL);
		glColorMaterial(GL_FRONT,GL_DIFFUSE);
	...
	switch(geometryArrayType)
	{
		...
		case POLYGON_ARRAY:glBegin(GL_POLYGON);break;
	}
	for(int i=0;i<vertexCount;i++)
	{
		...
		if(colors!=null)
		{
			...
			glColor4f(color->red,color->green,color->blue,color_alpha);
		}
		if(normals!=null)
		{
...
void glutGeometryStripArray(GeometryInfo* geometryInfo)
{
	...
	for(int i=0;i<stripCountsLength;i++)
	{
		switch(geometryArrayType)
		{
			...
			case POLYGON_STRIP_ARRAY:glBegin(GL_POLYGON);break;
		}
