...
double** Ytx(double* Yx0,int m,double* Yt0,int n)
{
	int i,j;
	double** Y=newDouble(m,n);
	for(j=0;j<n;j++)Y[0][j]=Yt0[j];
	...
	for(i=1;i<m;i++)
	{
		for(j=1;j<n;j++)
		{
			Y[i][j]=Y[i-1][j-1]+(Y[i-1][j]-Y[i-1][j-1])+(Y[i][j-1]-Y[i-1][j-1]);
		}
...
double** glutGetEquation()
{
	...
	double* Yx0=sin(0.01,20.0*PI,0,t,surfaceHeight);
	double* Yt0=sin(0.00,5*PI,0,x,surfaceWidth);
	return Ytx(c,Yx0,surfaceHeight,Yt0,surfaceWidth);
}
