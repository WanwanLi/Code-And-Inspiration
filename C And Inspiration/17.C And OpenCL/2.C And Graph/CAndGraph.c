...
int main()
{
	int i, j, row=1000, col=1000, dim=3;
	float* inputData=newFloat32Array(row*col);
	...
	cl_mem inputBuffer=clCreateKernelBuffer(inputData, row*dim);
	cl_mem outputBuffer=clCreateKernelBuffer(NULL, row*col);
	clSetKernelArgBuffer(inputBuffer);
