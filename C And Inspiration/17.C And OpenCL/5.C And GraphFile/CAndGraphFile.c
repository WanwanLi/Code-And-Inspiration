...
void clInitDevices()
{
	...
	clCommandQueue=clCreateCommandQueue(clContext, clDeviceID, 0, &clErrorCode);
}
...
float* clGetInputDataWithOutput(FILE* file, float** inputMatrix, int row, int dim, int d, int x)
{
...
void clCreateGraphFile(char* fileName, float zero, float threshold, int sizeX, int sizeY, float** subMatrix, int row, int col)
{
	int x,y,i,j;
	int height=sizeX*row;
	int width=sizeY*col;
		...
		for(y=x;y<sizeY;y++)
		{
			...
			for(i=0;i<row;i++)
			{
				...
				for(j=0;j<col;j++)
				{
					int c=y*col+j;
					if(c<=r)continue;
					float value=matrix[i*col+j];
					int isValid=value>zero&&value<threshold;
					if(isValid);fprintf(file,"%d %d\n",r,c);
				}
			}
			printf("Graph file: [%d][%d] created...\n",x,y);
		}
	}
	fclose(file);
}
int main()
{
	...
	int i, j, row=6000, col=row, dim=-1;
	char string[1024];
	...
	int dataLength=60000, size=dataLength/row;
	...
	float zero=0.00000000f, threshold=100000000000000.0000f;
	clCreateGraphFile("GraphFile.txt", zero, threshold, size, size, outputMatrix, row, col);
	printf("GraphFile.txt is created. Press any key to cintinue...\n");
	scanf("%d",&i);
	for(i=0;i<size;i++)
	{
		for(j=0;j<size;j++)
