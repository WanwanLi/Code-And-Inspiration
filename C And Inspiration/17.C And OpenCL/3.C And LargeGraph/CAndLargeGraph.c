...
float* clGetInputData(float** inputMatrix, int row, int dim, int d, int x)
{
...
void clExecuteFunc(float** inputMatrix, float** outputMatrix, int size, int row, int col, int dim)
{
	int x,y,i,j;
	...
	for(i=0,x=0;i<size;x+=row,i++)
	{
		...
		{
			float* inputData1=clGetInputData(inputMatrix, row, dim, i, x);
			float* inputData2=clGetInputData(inputMatrix, col, dim, j, y);
